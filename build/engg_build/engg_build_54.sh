export sourceDir="/var/lib/jenkins/workspace/BE540/5.4"
rm -rf /var/lib/jenkins/workspace/BE540/5.4/build/Enterprise/
cd ${sourceDir}/build
...
export logsDir="/var/lib/jenkins/workspace/BE540/logs/be"
...
export buildersemailaddr="mgujrath@tibco.com,bgokhale@tibco.com,sbagi@tibco.com,vasharma@tibco.com,rjain@tibco.com"

export punebuildersemailaddr="mgujrath@tibco.com,bgokhale@tibco.com,vasharma@tibco.com,rjain@tibco.com"
...
then
	...
	then
		...
		mail ${devqaemailaddr} -s "BE 5.4.0 daily Engineering Build - ${buildNum} - ${autobuild} Build will not proceed (No new checkins)"
...
#	export otherFiles=`svn log --non-interactive --username=qlbuild --password='T1bc0!23' -v -r $currBrHd:$lastSuccessRevNo|grep 'branches/5.4' |grep -v -e '_.*Version\.java'|grep -v 'vbuilds.log' | grep -v 'version.properties' |grep -v 'build/jenkins' |grep -v -e '_.*FeatureConfig\.xml' |grep -v 'about.mappings'`
	...
	then
	    ...
	    echo "$otherFiles" | mail ${punebuildersemailaddr} -s "BE 5.4.0 daily Engineering Build - ${buildNum} - ${autobuild} Build will not proceed (No new checkins)"
...
echo "$toBuild" | mail ${punebuildersemailaddr} -s "BE 5.4.0 daily Engineering Build - ${buildNum} - ${autobuild} Build started by $1 (Checkins detected)"

mail ${devqaemailaddr} -s "BE 5.4.0 daily Engineering Build - ${buildNum} - ${autobuild} Build started by $1 (Checkins detected)"
...
export bomBaseDir="${sourceDir}/build/pkg_info/bom/5.4.0"
