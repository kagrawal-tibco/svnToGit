export sourceDir="/var/lib/jenkins/workspace/BEMAIN/be-main"
rm -rf /var/lib/jenkins/workspace/BEMAIN/be-main/build/Enterprise/
cd ${sourceDir}/build
...
export logsDir="/var/lib/jenkins/workspace/BEMAIN/logs/be"

export releaseDir="/ql/build/BE/${major}.${minor}"

export assembliesExportDir="/mnt/ql/build/BE/leo/packaging/${version}-main"
...
then
	...
	then
		...
		mailx -s "BE 6.0.0 Engineering Build - ${buildNum} - ${autobuild} Build will not proceed (No new checkins)" ${devqaemailaddr} < /dev/null
...
#	export otherFiles=`svn log --non-interactive --username=qlbuild --password='T1bc0!23' -v -r $currBrHd:$lastSuccessRevNo|grep 'trunk/be-main' |grep -v -e '_.*Version\.java'|grep -v 'vbuilds.log' | grep -v 'version.properties' |grep -v 'build/jenkins' |grep -v -e '_.*FeatureConfig\.xml' |grep -v 'about.mappings'`
	...
	then
		...
		export otherFiles=`svn log --non-interactive --username=qlbuild --password='T1bc0!23' -v -r $currBrHd:$lastSuccessRevNo|grep 'trunk/be-main'|sort -u`	

	    echo "$otherFiles" | mailx -s "BE 6.0.0 daily Engineering Build - ${buildNum} - ${autobuild} Build will not proceed (No new checkins)" ${punebuildersemailaddr} 
...
#export changedFiles=`svn log --non-interactive --username=qlbuild --password='T1bc0!23' -v -r $currBrHd:$lastSuccessRevNo|grep 'trunk/be-main' |grep -v -e '_.*Version\.java'|grep -v 'vbuilds.log' | grep -v 'version.properties' |grep -v 'build/jenkins' |grep -v -e '_.*FeatureConfig\.xml' |grep -v 'about.mappings'`
...
echo "$toBuild" | mailx -s "BE 6.0.0 Engineering Build - ${buildNum} - ${autobuild} Build started by $1 (Checkins detected)" ${punebuildersemailaddr}

mailx -s "BE 6.0.0 Engineering Build - ${buildNum} - ${autobuild} Build started by $1 (Checkins detected)" ${devqaemailaddr} < /dev/null
...
export bomBaseDir="${sourceDir}/build/pkg_info/bom/6.0.0"
...
if [ "${buildRC}" = "" ];  then
   ...
   mailx -s "ATTENTION: BE ${buildNum} Engineering Build - Compilation failed" ${devqaemailaddr} < mailtext.txt
   cd ${sourceDir}/build
...
time /mnt/tsi/root/external/ant/1.10.6/bin/ant build-all -DqaTests=true > ${logsDir}/status_${buildNum}_installer.log
...
if [ "${buildInst}" != "" ];  then
   ...
   mailx -s "ATTENTION: BE ${version} Engineering Build - Installer Creation failed" ${devqaemailaddr} < ${logsDir}/status_${buildNum}_installer.log
   cd ${sourceDir}/build
   rm -f version.properties
   mv version.properties.backup version.properties
   cd ${sourceDir}/build/engg_build
