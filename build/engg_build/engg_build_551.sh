export sourceDir="/var/lib/jenkins/workspace/BE550/5.5"
rm -rf /var/lib/jenkins/workspace/BE550/5.5/build/Enterprise/
cd ${sourceDir}/build
...
export logsDir="/var/lib/jenkins/workspace/BE550/logs/be"
...
export buildersemailaddr="rjadhav@tibco.com,rjain@tibco.com,dijadhav@tibco.com"

export punebuildersemailaddr="bgokhale@tibco.com,pdeokar@tibco.com,rjain@tibco.com,dijadhav@tibco.com,rjadhav@tibco.com"
...
then
	...
	then
		...
		mail ${devqaemailaddr} -s "BE 5.5.1 daily Engineering Build - ${buildNum} - ${autobuild} Build will not proceed (No new checkins)"
...
#	export otherFiles=`svn log --non-interactive --username=qlbuild --password='T1bc0!23' -v -r $currBrHd:$lastSuccessRevNo|grep 'branches/5.5' |grep -v -e '_.*Version\.java'|grep -v 'vbuilds.log' | grep -v 'version.properties' |grep -v 'build/jenkins' |grep -v -e '_.*FeatureConfig\.xml' |grep -v 'about.mappings'`
	...
	then
		...
		export otherFiles=`svn log --non-interactive --username=qlbuild --password='T1bc0!23' -v -r $currBrHd:$lastSuccessRevNo|grep 'branches/5.5'|sort -u`	

	    echo "$otherFiles" | mail ${punebuildersemailaddr} -s "BE 5.5.1 daily Engineering Build - ${buildNum} - ${autobuild} Build will not proceed (No new checkins)"
...
#export changedFiles=`svn log --non-interactive --username=qlbuild --password='T1bc0!23' -v -r $currBrHd:$lastSuccessRevNo|grep 'branches/5.5' |grep -v -e '_.*Version\.java'|grep -v 'vbuilds.log' | grep -v 'version.properties' |grep -v 'build/jenkins' |grep -v -e '_.*FeatureConfig\.xml' |grep -v 'about.mappings'`
...
echo "$toBuild" | mail ${punebuildersemailaddr} -s "BE 5.5.1 daily Engineering Build - ${buildNum} - ${autobuild} Build started by $1 (Checkins detected)"

mail ${devqaemailaddr} -s "BE 5.5.1 daily Engineering Build - ${buildNum} - ${autobuild} Build started by $1 (Checkins detected)"
...
export bomBaseDir="${sourceDir}/build/pkg_info/bom/5.5.1"
...
if [ "${buildRC}" = "" ];  then
   ...
   mail ${buildersemailaddr} -s "ATTENTION: BE ${buildNum} Engineering Build - Compilation failed" < mailtext.txt
   cd ${sourceDir}/build
...
time /mnt/tsi/root/external/ant/1.8.1/bin/ant build-all-sp -DqaTests=true > ${logsDir}/status_${buildNum}_installer.log
