...
then
	...
	then
		...
		mail ${devqaemailaddr} -s "BE 5.5.0 daily Engineering Build - ${buildNum} - ${autobuild} Build will not proceed (No new checkins)"
...
#	export otherFiles=`svn log --non-interactive --username=qlbuild --password='T1bc0!23' -v -r $currBrHd:$lastSuccessRevNo|grep 'branches/5.5' |grep -v -e '_.*Version\.java'|grep -v 'vbuilds.log' | grep -v 'version.properties' |grep -v 'build/jenkins' |grep -v -e '_.*FeatureConfig\.xml' |grep -v 'about.mappings'`
	...
	then
	    ...
	    echo "$otherFiles" | mail ${punebuildersemailaddr} -s "BE 5.5.0 daily Engineering Build - ${buildNum} - ${autobuild} Build will not proceed (No new checkins)"
...
echo "$toBuild" | mail ${punebuildersemailaddr} -s "BE 5.5.0 daily Engineering Build - ${buildNum} - ${autobuild} Build started by $1 (Checkins detected)"

mail ${devqaemailaddr} -s "BE 5.5.0 daily Engineering Build - ${buildNum} - ${autobuild} Build started by $1 (Checkins detected)"
...
export bomBaseDir="${sourceDir}/build/pkg_info/bom/5.5.0"
