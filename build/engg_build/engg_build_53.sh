export sourceDir="/var/lib/jenkins/workspace/BE530/5.3"
rm -rf /var/lib/jenkins/workspace/BE530/5.3/build/Enterprise/
cd ${sourceDir}/build
...
export logsDir="/var/lib/jenkins/workspace/BE530/logs/be"
...
export punebuildersemailaddr="mgujrath@tibco.com,bgokhale@tibco.com,vasharma@tibco.com"

export tempmailinglist="mgujrath@tibco.com,bgokhale@tibco.com,vasharma@tibco.com,sbagi@tibco.com,be-qa@tibco.com"

export devqaemailaddr="businesseventsengineering-pune@tibco.com,be-qa@tibco.com,aamaya@tibco.com,fildiz@tibco.com,anpatil@tibco.com,rhollom@tibco.com"

export successemailaddr="businesseventsengineering-pune@tibco.com,be-qa@tibco.com,aamaya@tibco.com,fildiz@tibco.com,anpatil@tibco.com,rhollom@tibco.com,akunchef@tibco.com,kbalagur@tibco.com,mqian@tibco.com,magrawal@tibco.com,sjahagir@tibco.com,rshroff@tibco.com"
...
then
	...
	then
		...
		mail ${devqaemailaddr} -s "BE 5.3.0 daily Engineering Build - ${buildNum} - ${autobuild} Build will not proceed (No new checkins)"
...
#	export otherFiles=`svn log --non-interactive --username=qlbuild --password='T1bc0!23' -v -r $currBrHd:$lastSuccessRevNo|grep 'branches/5.3' |grep -v -e '_.*Version\.java'|grep -v 'vbuilds.log' | grep -v 'version.properties' |grep -v 'build/jenkins' |grep -v -e '_.*FeatureConfig\.xml' |grep -v 'about.mappings'`
	...
	then
		...
		export otherFiles=`svn log --non-interactive --username=qlbuild --password='T1bc0!23' -v -r $currBrHd:$lastSuccessRevNo|grep 'branches/5.3'|sort -u`	

	    echo "$otherFiles" | mail ${tempmailinglist} -s "BE 5.3.0 daily Engineering Build - ${buildNum} - ${autobuild} Build will not proceed (No new checkins)"
...
#export changedFiles=`svn log --non-interactive --username=qlbuild --password='T1bc0!23' -v -r $currBrHd:$lastSuccessRevNo|grep 'branches/5.3' |grep -v -e '_.*Version\.java'|grep -v 'vbuilds.log' | grep -v 'version.properties' |grep -v 'build/jenkins' |grep -v -e '_.*FeatureConfig\.xml' |grep -v 'about.mappings'`
...
echo "$toBuild" | mail ${punebuildersemailaddr} -s "BE 5.3.0 daily Engineering Build - ${buildNum} - ${autobuild} Build started by $1 (Checkins detected)"

mail ${devqaemailaddr} -s "BE 5.3.0 daily Engineering Build - ${buildNum} - ${autobuild} Build started by $1 (Checkins detected)"
...
/mnt/tsi/root/external/ant/1.8.1/bin/ant -f build.xml clobber-all pre-all

cd ${sourceDir}/build

export bomBaseDir="${sourceDir}/build/pkg_info/bom/5.3.0"
...
time /mnt/tsi/root/external/ant/1.8.1/bin/ant -f build.xml clobber-all build-all build-studio-site > ${logsDir}/status_${buildNum}_build.log
...
if [ "${buildRC}" = "" ];  then
   ...
   mail ${devqaemailaddr} -s "ATTENTION: BE ${buildNum} Engineering Build - Compilation failed" < mailtext.txt
   cd ${sourceDir}/build
...
mail ${devqaemailaddr} -s "BE Engineering Installer Build - ${buildNum} ${bebuild} - Compilation successful" < build.mail
...
time /mnt/tsi/root/external/ant/1.8.1/bin/ant build-all -DqaTests=true > ${logsDir}/status_${buildNum}_installer.log
...
if [ "${buildInst}" != "" ];  then
   ...
   mail ${punebuildersemailaddr} -s "ATTENTION: BE ${version} Engineering Build - Installer Creation failed" < ${logsDir}/status_${buildNum}_installer.log
   cd ${sourceDir}/build
   rm -f version.properties
   mv version.properties.backup version.properties
   cd ${sourceDir}/build/engg_build
...
#asJarPath=`/mnt/tsi/root/external/ant/1.8.1/bin/ant -f build.xml showenv-as | awk '/AS_LIB/ {print $4}' | tr -d "[]" | awk '{printf("%s/as-common.jar",$1)}'`
#echo $asJarPath
...
mail ${successemailaddr} -s "BE Engg Build- BE ${buildNum}" < build.mail
