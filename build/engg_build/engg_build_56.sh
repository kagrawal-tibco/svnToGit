export sourceDir="/var/lib/jenkins/workspace/BE560/5.6"
rm -rf /var/lib/jenkins/workspace/BE560/5.6/build/Enterprise/
cd ${sourceDir}/build
...
export logsDir="/var/lib/jenkins/workspace/BE560/logs/be"
...
export buildersemailaddr="vshende@tibco.com,rjain@tibco.com,dijadhav@tibco.com,rjadhav@tibco.com,vshelar@tibco.com"

export punebuildersemailaddr="vshende@tibco.com,rjain@tibco.com,dijadhav@tibco.com,rjadhav@tibco.com,vshelar@tibco.com"

export devqaemailaddr="stg-engr@tibco.com,be-qa@tibco.com"

export successemailaddr="stg-engr@tibco.com,be-qa@tibco.com,kbalagur@tibco.com,mqian@tibco.com,magrawal@tibco.com"

export failureemailaddr="stg-engr@tibco.com,vpatil@tibco.com,rhollom@tibco.com,vshelar@tibco.com"
...
then
	...
	then
		...
		mail ${devqaemailaddr} -s "BE 5.6.0 daily Engineering Build - ${buildNum} - ${autobuild} Build will not proceed (No new checkins)"
...
#	export otherFiles=`svn log --non-interactive --username=qlbuild --password='T1bc0!23' -v -r $currBrHd:$lastSuccessRevNo|grep 'branches/5.6' |grep -v -e '_.*Version\.java'|grep -v 'vbuilds.log' | grep -v 'version.properties' |grep -v 'build/jenkins' |grep -v -e '_.*FeatureConfig\.xml' |grep -v 'about.mappings'`
	...
	then
		...
		export otherFiles=`svn log --non-interactive --username=qlbuild --password='T1bc0!23' -v -r $currBrHd:$lastSuccessRevNo|grep 'branches/5.6'|sort -u`	

	    echo "$otherFiles" | mail ${punebuildersemailaddr} -s "BE 5.6.0 daily Engineering Build - ${buildNum} - ${autobuild} Build will not proceed (No new checkins)"
...
#export changedFiles=`svn log --non-interactive --username=qlbuild --password='T1bc0!23' -v -r $currBrHd:$lastSuccessRevNo|grep 'branches/5.6' |grep -v -e '_.*Version\.java'|grep -v 'vbuilds.log' | grep -v 'version.properties' |grep -v 'build/jenkins' |grep -v -e '_.*FeatureConfig\.xml' |grep -v 'about.mappings'`
...
echo "$toBuild" | mail ${punebuildersemailaddr} -s "BE 5.6.0 daily Engineering Build - ${buildNum} - ${autobuild} Build started by $1 (Checkins detected)"

mail ${devqaemailaddr} -s "BE 5.6.0 daily Engineering Build - ${buildNum} - ${autobuild} Build started by $1 (Checkins detected)"
...
export bomBaseDir="${sourceDir}/build/pkg_info/bom/5.6.0"
...
if [ "${buildRC}" = "" ];  then
   ...
   mail ${failureemailaddr} -s "ATTENTION: BE ${buildNum} Engineering Build - Compilation failed" < mailtext.txt
   cd ${sourceDir}/build
...
if [ "${buildInst}" != "" ];  then
   ...
   mail ${buildersemailaddr} -s "ATTENTION: BE ${version} Engineering Build - Installer Creation failed" < ${logsDir}/status_${buildNum}_installer.log
   cd ${sourceDir}/build
   rm -f version.properties
   mv version.properties.backup version.properties
   cd ${sourceDir}/build/engg_build
