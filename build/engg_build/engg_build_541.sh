...
export buildersemailaddr="pdeokar@tibco.com,rjain@tibco.com"

export punebuildersemailaddr="bgokhale@tibco.com,pdeokar@tibco.com,rjain@tibco.com"
...
then
	...
	then
		...
		mail ${devqaemailaddr} -s "BE 5.4.1 daily Engineering Build - ${buildNum} - ${autobuild} Build will not proceed (No new checkins)"
...
#	export otherFiles=`svn log --non-interactive --username=qlbuild --password='T1bc0!23' -v -r $currBrHd:$lastSuccessRevNo|grep 'branches/5.4' |grep -v -e '_.*Version\.java'|grep -v 'vbuilds.log' | grep -v 'version.properties' |grep -v 'build/jenkins' |grep -v -e '_.*FeatureConfig\.xml' |grep -v 'about.mappings'`
	...
	then
	    ...
	    echo "$otherFiles" | mail ${punebuildersemailaddr} -s "BE 5.4.1 daily Engineering Build - ${buildNum} - ${autobuild} Build will not proceed (No new checkins)"
...
echo "$toBuild" | mail ${punebuildersemailaddr} -s "BE 5.4.1 daily Engineering Build - ${buildNum} - ${autobuild} Build started by $1 (Checkins detected)"

mail ${devqaemailaddr} -s "BE 5.4.1 daily Engineering Build - ${buildNum} - ${autobuild} Build started by $1 (Checkins detected)"
