...
export logsDir="/var/lib/jenkins/workspace/BE522/logs/be"
...
then
	...
	then
		mail ${buildersemailaddr} -s "BE 5.2.2 daily Engineering Build - ${buildNum} - ${autobuild} Build will not proceed (No new checkins)"
   		echo "Exiting build....."
   		exit 1
	fi
	cd ${sourceDir}/build
...
#	export otherFiles=`svn log --non-interactive --username=qlbuild --password=T!bco123 -v -r $currBrHd:$lastSuccessRevNo|grep 'branches/5.2' |grep -v -e '_.*Version\.java'|grep -v 'vbuilds.log' | grep -v 'version_sp.properties' |grep -v 'build/jenkins' |grep -v -e '_.*FeatureConfig\.xml' |grep -v 'about.mappings'`
	...
	then
	    ...
	    echo "$otherFiles" | mail ${buildersemailaddr} -s "BE 5.2.2 daily Engineering Build - ${buildNum} - ${autobuild} Build will not proceed (No new checkins)"
   		echo "Exiting build....."
   		exit 1
	fi
fi
...
echo "toBuild.." $toBuild
echo "$toBuild" | mail ${punebuildersemailaddr} -s "BE 5.2.2 daily Engineering Build - ${buildNum} - ${autobuild} Build started by $1 (Checkins detected)"
mail ${devqaemailaddr} -s "BE 5.2.2 daily Engineering Build - ${buildNum} - ${autobuild} Build started by $1 (Checkins detected)"
...
cd ${sourceDir}/build
export bomBaseDir="${sourceDir}/build/pkg_info/bom/5.2.2"
export enggDir="${sourceDir}/build/engg_build"
cd ${bomBaseDir}
...
if [ "${buildRC}" = "" ];  then
   ...
   mail ${devqaemailaddr} -s "ATTENTION: BE ${version} Engineering Build - Compilation failed" < ${logsDir}/status_${buildNum}_build.log
   cd ${sourceDir}/build
   rm -f version_sp.properties
   mv version_sp.properties.backup version_sp.properties
   rm -f vbuilds.log
   mv vbuilds.log.backup vbuilds.log
svn commit --non-interactive --username=qlbuild --password=T!bco123 ${sourceDir}/build/engg_build/vbuilds.log ${sourceDir}/build/version_sp.properties -m "reset"
   exit 127
fi
