export sourceDir="/var/lib/jenkins/workspace/BE541-HF/5.4.1-HF"
rm -rf /var/lib/jenkins/workspace/BE541-HF/5.4.1-HF/build/Enterprise/
cd ${sourceDir}/build
...
export logsDir="/var/lib/jenkins/workspace/BE541-HF/logs/be"
...
export buildersemailaddr="vshelar@tibco.com,rjain@tibco.com,rjadhav@tibco.com"
export punebuildersemailaddr="vshelar@tibco.com,rjain@tibco.com,rjadhav@tibco.com"
export devqaemailaddr="vshelar@tibco.com,rjain@tibco.com,rjadhav@tibco.com"
...
export successemailaddr="be-qa-engr@tibco.com,businesseventsengineering-pune@tibco.com,magrawal@tibco.com"
...
then
	...
	then
		...
		mail ${devqaemailaddr} -s "BE 5.4.1 HF3 Engineering Build - ${buildNum} - ${autobuild} Build will not proceed (No new checkins)"
...
#	export otherFiles=`svn log --non-interactive --username=qlbuild --password='T1bc0!23' -v -r $currBrHd:$lastSuccessRevNo|grep 'branches/5.4' |grep -v -e '_.*Version\.java'|grep -v 'vbuilds.log' | grep -v 'version.properties' |grep -v 'build/jenkins' |grep -v -e '_.*FeatureConfig\.xml' |grep -v 'about.mappings'`
	...
	then
		...
		export otherFiles=`svn log --non-interactive --username=qlbuild --password='T1bc0!23' -v -r $currBrHd:$lastSuccessRevNo|grep 'branches/5.4'|sort -u`	

	    echo "$otherFiles" | mail ${punebuildersemailaddr} -s "BE 5.4.1 HF3 Engineering Build - ${buildNum} - ${autobuild} Build will not proceed (No new checkins)"
...
#export changedFiles=`svn log --non-interactive --username=qlbuild --password='T1bc0!23' -v -r $currBrHd:$lastSuccessRevNo|grep 'branches/5.4' |grep -v -e '_.*Version\.java'|grep -v 'vbuilds.log' | grep -v 'version.properties' |grep -v 'build/jenkins' |grep -v -e '_.*FeatureConfig\.xml' |grep -v 'about.mappings'`
...
echo "$toBuild" | mail ${punebuildersemailaddr} -s "BE 5.4.1 HF3 Engineering Build - ${buildNum} - ${autobuild} Build started by $1 (Checkins detected)"

mail ${devqaemailaddr} -s "BE 5.4.1 HF3 Engineering Build - ${buildNum} - ${autobuild} Build started by $1 (Checkins detected)"
...
/mnt/tsi/root/external/ant/1.8.1/bin/ant -f build.xml clobber-all pre-all-hf

cd ${sourceDir}/build

export bomBaseDir="${sourceDir}/build/pkg_info/bom/5.4.1"
...
time /mnt/tsi/root/external/ant/1.8.1/bin/ant -f build.xml clobber-all build-all-hf build-studio-site > ${logsDir}/status_${buildNum}_build.log
...
time /mnt/tsi/root/external/ant/1.8.1/bin/ant build-hf -DqaTests=false > ${logsDir}/status_${buildNum}_installer.log
...
if [ "${buildInst}" != "" ];  then
   ...
   mail ${devqaemailaddr} -s "ATTENTION: BE ${version} Engineering Build - Installer Creation failed" < ${logsDir}/status_${buildNum}_installer.log
   cd ${sourceDir}/build
   rm -f version.properties
   mv version.properties.backup version.properties
   cd ${sourceDir}/build/engg_build
