...
public class BeHfChangesProvider
        extends BeChangesProvider {


    public BeHfChangesProvider() {
    ...
    protected void addAssemblyPathMappings(
        ...
        if (changedFileSets.isEmpty()) {
            ...
            for (final FileSet fileSet : changedFileSets) {
                ...
                final String pathForDir = isDir ? fileSet.getInstalledHfPathFromSourcePath(changedPath) : null;
                for (final String installedPath : fileSet.getInstalledHfPaths()) {
                    for (final Assembly assembly : knownAssemblies.getByInstalledHfPath(installedPath)) {
    ...
    public static void main(
        ...
        System.err.println("Finding HF file(s) updated between " + start + " and " + end + ".");

        boolean first = true;
        for (final Map.Entry<String, SortedSet<String>> nameToPaths :
                new BeHfChangesProvider().getChanges(start, end).entrySet()) {
            if (first) {
                first = false;
            } else {
                System.out.println();
