...
@Mojo( name = "bedeploy", defaultPhase = LifecyclePhase.DEPLOY, threadSafe = true )
    ...
    extends AbstractDeployMojo
{
        ...
        throws MojoExecutionException, MojoFailureException
    {
        ...
        boolean isEARArtifact = "ear".equals( packaging );
        if ( !isPomArtifact && !isEARArtifact)
        {
        ...
        try
        {
            if ( isPomArtifact || isEARArtifact )
            {
            	...
            	if(new File(earBuildPath).isFile()){ //deploy project lib
                	Artifact earArtifact =artifactFactory.createBuildArtifact( artifact.getGroupId(), artifact.getArtifactId(),
                                                                   artifact.getBaseVersion(),"ear" );
                	earArtifact.setFile( new File(earBuildPath) );
                	deploy( new File(earBuildPath), earArtifact, repo, getLocalRepository(), retryFailedDeploymentCount );
                }
                ...
                if(new File(projectLibPath).isFile()){ //deploy project lib
                	Artifact projectLibArtifact =artifactFactory.createBuildArtifact( artifact.getGroupId(), artifact.getArtifactId(),
                                                                   artifact.getBaseVersion(),"projlib" );
                	projectLibArtifact.setFile( new File(projectLibPath) );
                	deploy( new File(projectLibPath), projectLibArtifact, repo, getLocalRepository(), retryFailedDeploymentCount );
                }
