...
public class ContentSettingsFilter implements IFilter {

	@Override
	public boolean select(Object toTest) {
		if (toTest instanceof LocalStateVisualization){
			try {
				if (StateMachineComponentHelper.getContentSeriesConfig((LocalStateVisualization) toTest) != null){
					return true;
				}
			} catch (Exception e) {
				IStatus status = new Status(IStatus.ERROR,DashboardStateMachineComponentPlugin.PLUGIN_ID,"could not find content series config in "+toTest,e);
