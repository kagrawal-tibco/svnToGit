...
public class GenerateEventDiagramActionDelegate implements IWorkbenchWindowActionDelegate,IObjectActionDelegate {
	...
	public void run(IAction action) {
		...
		try {
			if(_selection != null && _selection instanceof IStructuredSelection){
				...
				final IFile projectViewFile = project.getFile(project.getName()+ ".eventview");
				if(projectViewFile.exists()){
					...
					if(workSpace != null){
						...
						File mfile = new File(projectPath + "/"+ project.getName()+ ".eventview");
						if (!mfile.exists()) {
							try {
								mfile.createNewFile();
							} catch (Exception e) {
