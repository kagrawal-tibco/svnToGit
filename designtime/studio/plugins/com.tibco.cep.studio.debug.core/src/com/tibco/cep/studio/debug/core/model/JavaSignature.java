...
public class JavaSignature {
	...
	public static char[] createTypeParameterSignature(char[] typeParameterName, char[][] boundSignatures) {
		...
		if (length == 0) {
			return CharUtil.append(typeParameterName, C_COLON); // param signature with no bounds still gets trailing colon
		}
		int boundsSize = 0;
	...
	public static char[] createCharArrayTypeSignature(char[] typeName, boolean isResolved) {
		if (typeName == null) throw new IllegalArgumentException("null"); 
	...
	private static int consumeWhitespace(char[] typeName, int pos, int length) {
	    while (pos < length) {
	        ...
	        if (currentChar != ' ' && !CharUtil.isWhitespace(currentChar)) {
	            break;
	        }
	        pos++;
	    }
	...
	private static int encodeTypeSignature(char[] typeName, int start, boolean isResolved, int length, StringBuffer buffer) {
	    ...
	    switch (currentChar) {
			// primitive type?
			case 'b' :
			    checkPos = CharUtil.checkName(BOOLEAN, typeName, pos, length);
			    if (checkPos > 0) {
			        pos = encodeArrayDimension(typeName, checkPos, length, buffer);
				    buffer.append(C_BOOLEAN);
				    return pos;
				} 
			    checkPos = CharUtil.checkName(BYTE, typeName, pos, length);
			    if (checkPos > 0) {
			        pos = encodeArrayDimension(typeName, checkPos, length, buffer);
			    ...
			    break;
			case 'd':
			    checkPos = CharUtil.checkName(DOUBLE, typeName, pos, length);
			    if (checkPos > 0) {
			        pos = encodeArrayDimension(typeName, checkPos, length, buffer);
			    ...
			    break;
			case 'f':
			    checkPos = CharUtil.checkName(FLOAT, typeName, pos, length);
			    if (checkPos > 0) {
			        pos = encodeArrayDimension(typeName, checkPos, length, buffer);
			    ...
			    break;
			case 'i':
			    checkPos = CharUtil.checkName(INT, typeName, pos, length);
			    if (checkPos > 0) {
			        pos = encodeArrayDimension(typeName, checkPos, length, buffer);
				    buffer.append(C_INT);
				    return pos;
			    ...
			    break;
			case 'l':
			    checkPos = CharUtil.checkName(LONG, typeName, pos, length);
			    if (checkPos > 0) {
			        pos = encodeArrayDimension(typeName, checkPos, length, buffer);
			    ...
			    break;
			case 's':
			    checkPos = CharUtil.checkName(SHORT, typeName, pos, length);
			    if (checkPos > 0) {
			        pos = encodeArrayDimension(typeName, checkPos, length, buffer);
			    ...
			    break;
			case 'v':
			    checkPos = CharUtil.checkName(VOID, typeName, pos, length);
			    if (checkPos > 0) {
			        pos = encodeArrayDimension(typeName, checkPos, length, buffer);
			    ...
			    break;
			case 'c':
			    checkPos = CharUtil.checkName(CHAR, typeName, pos, length);
			    if (checkPos > 0) {
			        pos = encodeArrayDimension(typeName, checkPos, length, buffer);
				    buffer.append(C_CHAR);
				    return pos;
				} else {
					checkPos = CharUtil.checkName(CAPTURE, typeName, pos, length);
					if (checkPos > 0) {
			...
			case '?':
				...
				checkPos = CharUtil.checkName(EXTENDS, typeName, pos, length);
				if (checkPos > 0) {
					buffer.append(C_EXTENDS);
					pos = encodeTypeSignature(typeName, checkPos, isResolved, length, buffer);
					return pos;
				}
				checkPos = CharUtil.checkName(SUPER, typeName, pos, length);
				if (checkPos > 0) {
	...
	public static int getParameterCount(char[] methodSignature) throws IllegalArgumentException {
		try {
			int count = 0;
			int i = CharUtil.indexOf(C_PARAM_START,methodSignature);
			if (i < 0) {
				throw new IllegalArgumentException();
			} else {
				i++;
			...
			for (;;) {
				...
				int e= SignatureUtil.scanTypeSignature(methodSignature, i);
				if (e < 0) {
					throw new IllegalArgumentException();
				} else {
					i = e + 1;
	...
	public static char[][] getParameterTypes(char[] methodSignature) throws IllegalArgumentException {
		try {
			...
			for (;;) {
				...
				result[t] = CharUtil.subarray(methodSignature, i, e + 1);
				t++;
				i = e + 1;
			}
		} catch (ArrayIndexOutOfBoundsException e) {
	...
	public static String[] getParameterTypes(String methodSignature) throws IllegalArgumentException {
		...
		return toStrings(parameterTypes);
	}
	...
	public static char[][] getThrownExceptionTypes(char[] methodSignature) throws IllegalArgumentException {
		...
		int exceptionStart = CharUtil.indexOf(C_EXCEPTION_START,methodSignature);
		if (exceptionStart == -1) {
			int paren = CharUtil.lastIndexOf(C_PARAM_END,methodSignature);
			if (paren == -1) {
				throw new IllegalArgumentException();
			}
			// ignore return type
			exceptionStart = SignatureUtil.scanTypeSignature(methodSignature, paren+1) + 1;
			int length = methodSignature.length;
			if (exceptionStart == length) return new char[0][];
			throw new IllegalArgumentException();
		}
		int length = methodSignature.length;
		...
		while (i < length) {
			...
			i = SignatureUtil.scanTypeSignature(methodSignature, i) + 1;
			exceptionList.add(CharUtil.subarray(methodSignature, exceptionStart,i));	
			exceptionStart = i;
		}
		char[][] result;
	...
	public static char[][] getTypeArguments(char[] parameterizedTypeSignature) throws IllegalArgumentException {
		...
		while (true) {
			...
			int e = SignatureUtil.scanTypeArgumentSignature(parameterizedTypeSignature, p);
			args.add(CharUtil.subarray(parameterizedTypeSignature, p, e+1));
			p = e + 1;
		}
	}
	...
	public static char[] getTypeErasure(char[] parameterizedTypeSignature) throws IllegalArgumentException {
		int end = CharUtil.indexOf(C_GENERIC_START,parameterizedTypeSignature);
	...
	public static char[][] getTypeParameters(char[] methodOrTypeSignature) throws IllegalArgumentException {
		try {
			...
			if (methodOrTypeSignature[0] != C_GENERIC_START) return new char[0][];
			...
			while (i < length) {
				...
				i = CharUtil.indexOf(C_COLON,methodOrTypeSignature,i);
				if (i < 0 || i >= length) 
					throw new IllegalArgumentException();
				// iterate over bounds
				while (methodOrTypeSignature[i] == ':') {
					...
					switch (methodOrTypeSignature[i]) {
						...
						case C_RESOLVED:
							try {
								i = SignatureUtil.scanClassTypeSignature(methodOrTypeSignature, i);
						...
						case C_ARRAY:
							try {
								i = SignatureUtil.scanArrayTypeSignature(methodOrTypeSignature, i);
								i++; // position at start of next param if any
							} catch (IllegalArgumentException e) {
						...
						case C_TYPE_VARIABLE:
							try {
								i = SignatureUtil.scanTypeVariableSignature(methodOrTypeSignature, i);
								i++; // position at start of next param if any
							} catch (IllegalArgumentException e) {
				...
				paramList.add(CharUtil.subarray(methodOrTypeSignature, paramStart, i));
	...
	public static char[] getTypeVariable(char[] formalTypeParameterSignature) throws IllegalArgumentException {
		int p = CharUtil.indexOf(C_COLON,formalTypeParameterSignature);
		if (p < 0) {
			// no ":" means can't be a formal type parameter signature
			throw new IllegalArgumentException();
		}
		return CharUtil.subarray(formalTypeParameterSignature, 0, p);
	}
	...
	public static char[][] getTypeParameterBounds(char[] formalTypeParameterSignature) throws IllegalArgumentException {
		int p1 = CharUtil.indexOf(C_COLON,formalTypeParameterSignature);
		if (p1 < 0) {
			// no ":" means can't be a formal type parameter signature
			throw new IllegalArgumentException();
		}
		...
		int p2 = CharUtil.indexOf(C_COLON,formalTypeParameterSignature,p1 + 1);
		char[] classBound;
		if (p2 < 0) {
			// no interface bounds
			classBound = CharUtil.subarray(formalTypeParameterSignature, p1 + 1, formalTypeParameterSignature.length);
		...
		if (p2 == p1 + 1) {
			...
			classBound = CharUtil.subarray(formalTypeParameterSignature, p1 + 1, p2);
		}
		char[][] interfaceBounds = CharUtil.splitOn(C_COLON, formalTypeParameterSignature, p2 + 1, formalTypeParameterSignature.length);
		if (classBound == null) {
	...
	public static char[] getQualifier(char[] name) {
		int firstGenericStart = CharUtil.indexOf(C_GENERIC_START,name);
		int lastDot = CharUtil.lastIndexOf(C_DOT, name, 0, firstGenericStart == -1 ? name.length-1 : firstGenericStart);
		if (lastDot == -1) {
			return new char[0];
		}
		return CharUtil.subarray(name, 0, lastDot);
	}
	/**
	...
	public static char[] getReturnType(char[] methodSignature) throws IllegalArgumentException {
		...
		int last = SignatureUtil.scanTypeSignature(methodSignature, paren+1);
		return CharUtil.subarray(methodSignature, paren + 1, last+1);
	}
	/**
	 * Extracts the return type from the given method signature. The method signature is 
	 * expected to be dot-based.
	...
	public static char[] getSignatureQualifier(char[] typeSignature) {
		if(typeSignature == null) return new char[0];
		
		char[] qualifiedType = JavaSignature.toCharArray(typeSignature);
		...
		if(dotCount > 0) {
			for(int i = 0; i < qualifiedType.length; i++) {
				...
				if(dotCount <= 0) {
					return CharUtil.subarray(qualifiedType, 0, i);
				}
			}
		}
		return new char[0];
	...
	public static char[] getSignatureSimpleName(char[] typeSignature) {
		...
		if(dotCount > 0) {
			for(int i = 0; i < qualifiedType.length; i++) {
				...
				if(dotCount <= 0) {
					return CharUtil.subarray(qualifiedType, i + 1, qualifiedType.length);
				}
			}
		}
	...
	public static char[] getSimpleName(char[] name) {
		...
		if (lastGenericStart < 0) {
			...
			return  CharUtil.subarray(name, lastDot + 1, length);
		}
	...
	private static void appendSimpleName(char[] name, int start, int end, StringBuffer buffer) {
		...
		if (name[start] == '?') { // wildcard
			...
			switch (name[index]) {
				case 'e' :
					int checkPos = CharUtil.checkName(EXTENDS, name, index, end);
				    if (checkPos > 0) {
					...
					break;
				case 's' :
					checkPos = CharUtil.checkName(SUPER, name, index, end+1);
				    if (checkPos > 0) {
	...
	public static char[] removeCapture(char[] methodOrTypeSignature) {
		return CharUtil.remove(methodOrTypeSignature, C_CAPTURE);
	}
	...
	public static char[] toCharArray(char[] methodSignature, char[] methodName, char[][] parameterNames, boolean fullyQualifyTypeNames, boolean includeReturnType, boolean isVargArgs) {
		int firstParen = CharUtil.indexOf(C_PARAM_START,methodSignature);
		if (firstParen == -1) {
			throw new IllegalArgumentException();
		}
	...
	public static char[] toCharArray(char[] signature) throws IllegalArgumentException {
			...
			if (sigLength == 0 || signature[0] == C_PARAM_START || signature[0] == C_GENERIC_START) {
				return toCharArray(signature, new char[0], null, true, true);
			}
	...
	private static int appendTypeSignature(char[] string, int start, boolean fullyQualifyTypeNames, StringBuffer buffer, boolean isVarArgs) {
		...
		if (isVarArgs) {
			...
			switch (c) {
				...
				case C_TYPE_VARIABLE :
					int e = SignatureUtil.scanTypeVariableSignature(string, start);
