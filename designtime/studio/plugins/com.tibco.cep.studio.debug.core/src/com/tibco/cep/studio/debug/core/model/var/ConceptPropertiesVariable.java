...
public class ConceptPropertiesVariable extends RuleDebugVariable {
	...
	public ConceptPropertiesVariable(
	...
	public void processChildren() {
		...
		try {
			Map<String,Value> propMap = DebuggerSupport.getConceptProperties(getThreadInfo(), getConceptRef());
	...
	private AbstractDebugVariable getPropertyAtomRow(Entry<String, Value> prop) throws Exception {
		...
		if (!DebuggerSupport.isConceptPropertyStateMachine(getThreadInfo(), (ObjectReference) prop.getValue())) {
			if (DebuggerSupport.isPropertyAtomConceptValue(getThreadInfo(), (ObjectReference) prop.getValue())) {
				if (DebuggerSupport.implementsInterface((ObjectReference) prop.getValue(), PropertyAtomConcept.class.getName())) {
					ObjectReference conReference = DebuggerSupport.getPropertyAtomConceptValue(getThreadInfo(), (ObjectReference) prop.getValue());
					var = new ConceptVariable(getStackFrame(),getThreadInfo(), conReference, propname, conReference);
				} 
			} else {
