...
public class QueryAgentNodeCreator extends TSNodeBuilder{
	...
	public TSENode addNode(TSEGraph graph) {
		ImageNodeUI ui = new ImageNodeUI(new TSEImage(this.getClass(),"/icons/queryagent_48x48.png"));
		TSENode node = super.addNode(graph);
		try {
			String tag = ClusterTopologyUtils.getNodeName(graph,ClusterTopologyUtils.UNIQUE_QUERY_AGENT);
			node.setName(tag);
			node.setAttribute(ClusterTopologyConstants.NODE_TYPE, Integer.valueOf(ClusterTopologyConstants.AGENT_TYPE_QUERY));
			node.setSize(48, 48);
