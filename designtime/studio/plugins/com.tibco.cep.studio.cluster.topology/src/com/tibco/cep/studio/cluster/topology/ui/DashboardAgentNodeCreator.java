...
public class DashboardAgentNodeCreator extends TSNodeBuilder {
	...
	public TSENode addNode(TSEGraph graph) {
		ImageNodeUI ui =new ImageNodeUI(new TSEImage(this.getClass(),"/icons/dashboardagent_48x48.png"));
		TSENode node = super.addNode(graph);
		try {
			String tag = ClusterTopologyUtils.getNodeName(graph,ClusterTopologyUtils.UNIQUE_DASHBORAD_AGENT);
			node.setName(tag);
			node.setAttribute(ClusterTopologyConstants.NODE_TYPE, Integer.valueOf(ClusterTopologyConstants.AGENT_TYPE_DASHBOARD));
			node.setSize(48, 48);
