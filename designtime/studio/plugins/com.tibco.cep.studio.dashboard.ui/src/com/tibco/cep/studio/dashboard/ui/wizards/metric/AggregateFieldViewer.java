...
import com.tibco.cep.studio.dashboard.ui.editors.metric.viewers.AggregationTypeSelector;
...
public class AggregateFieldViewer extends AttributeViewer {

	private static final String AGGREGATIONTYPE_COLUMN = "Aggregation Type";

	public AggregateFieldViewer(FormToolkit toolKit, Composite parent, boolean editToolbarSupported, String childType) {
	...
	public void createTable(Composite parent) {
		...
		setColumnNames(new String[] { NAME_COLUMN, AGGREGATIONTYPE_COLUMN, DATATYPE_COLUMN });
		...
		column.setText(AGGREGATIONTYPE_COLUMN);
		column.setToolTipText(AGGREGATIONTYPE_COLUMN);
		column.setResizable(true);
		tableLayout.addColumnData(new ColumnWeightData(1, true));
	...
	public void createTableViewer() {
		...
		//aggregation type editor
		editors[idx++] = new AggregateTypeCellEditor(getTable());
	...
	class SimpleAttributeLabelProvider extends LabelProvider implements ITableLabelProvider {
		...
		public String getColumnText(Object element, int columnIndex) {
			if (element instanceof LocalMetricField) {
				LocalMetricField localAttribute = (LocalMetricField) element;
				switch (columnIndex) {
					case 0:
						return localAttribute.getName();
					case 1:
						return localAttribute.getAggregationFunction();
					case 2:
				...
				try {
					...
					switch (columnIndex) {
						...
						case 1:
							...
							break;
						case 2:
							//do nothing the AggregateTypeCellEditor will take care of this
							break;
						default:
							break;
					}
	...
	class DataTypeCellEditor extends DialogCellEditor {
		...
		protected void updateContents(Object value) {
			String text = "";
			if (value != null) {
				LocalMetricField attribute = (LocalMetricField) getValue();
	...
	class AggregateTypeCellEditor extends DialogCellEditor {

		AggregateTypeCellEditor(Composite parent) {
			super(parent);
		}
		...
		protected void updateContents(Object value) {
			...
			if (value != null) {
				LocalMetricField attribute = (LocalMetricField) getValue();
				text = attribute.getAggregationFunction();
			}
			getDefaultLabel().setText(text);
		}
		...
		protected Object openDialogBox(Control cellEditorWindow) {
			LocalMetricField attribute = (LocalMetricField) getValue();
			AggregationTypeSelector dlg = new AggregationTypeSelector(cellEditorWindow.getShell(), attribute.getSupportedAggregateFuncs(), attribute.getAggregationFunction());
			dlg.open();
			final String newAggrType = (String) dlg.getFirstResult();
			if (newAggrType != null && newAggrType.equals(attribute.getDataType()) == false) {
				attribute.setAggregationFunction(newAggrType);
			}
			return attribute;
		}
