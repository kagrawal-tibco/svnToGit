package com.tibco.cep.studio.mapper.ui.jedit.syntaxcoloring;
 ...
 *
 * Tibco modified areas indicated by 'TIBCO' around it.
 */
...
import com.tibco.cep.studio.mapper.EventListenerList;
...
public class JEditTextArea extends JComponent
{
	...
	public JEditTextArea(TextAreaDefaults defaults)
	{
        ...
        //TIBCO modified --- allow painter to be substituted.
		painter = buildPainter(defaults);
        //end TIBCO modified --- allow painter to be substituted.
    ...
    /**
     * TIBCO Added to original jEdit code.
     * Builds the text area painter.<br>
     * Subclasses may override for a better painter.
     * @param defaults
     * @return The painter.
     */
...
//			Toolkit toolkit = painter.getToolkit();
	...
	public void setText(String text)
	{
		try
		{
			// for CR 1-8AF8EB by yzhao
			SyntaxDocument doc = new SyntaxDocument();
			doc.setTokenMarker(document.getTokenMarker());
			...
			{
				if(listeners[i] != documentHandler)
				{
					document.addDocumentListener(listeners[i]);
				}
			}
	...
	public void processKeyEvent(KeyEvent evt)
	{
        ...
        if (isFieldMode()) {
            switch (evt.getKeyCode()) {
                ...
                case KeyEvent.VK_ENTER:
                    // for these things, give listeners first crack.
        ...
        //TIBCO CHANGE
        if (!evt.isConsumed()) {
            super.processKeyEvent(evt); // allow listeners to have a swing at it.
        }
        // END TIBCO CHANGE
	}

    /**
     * TIBCO added -- allows a subclass of this to make jedit work properly as a single field.
     * @return true if in field mode, false by default.
     */
	...
	{
		CaretListener[] listeners = listenerList.getListeners(CaretListener.class);
        for (int i = 0; i < listeners.length; i++) {
        	listeners[i].caretUpdate(caretEvent);
		}
	}
	...
	class MouseHandler extends MouseAdapter
	{
			...
			int offset, int dot)
		{
			if((evt.getModifiers() & InputEvent.SHIFT_MASK) != 0)
			{
                ...
                // TIBCO CHANGE:
                if (dot>=getDocument().getLength()) {
                    dot = getDocument().getLength();
                }
                // END TIBCO CHANGE
    ...
    /**
     * TIBCO ADDED - Called by input handler when attempts are made to edit non-editable.<br>
    ...
    /**
     * TIBCO ADDED - Called by input handler when attempts are made to do bad editing things such as backspacing
     * through the beginning.  JEdit used to beep() which is obnoxious.<br>
     * Does nothing here.
     */
