...
public class TextAreaPainter extends JComponent implements TabExpander
{
	...
	public void paint(Graphics gfx)
	{
        ...
        // Added so that we can overpaint insets (for errors)
		...
		try
		{
            ...
            // Start TIBCO change
            for(int line = Math.max(0,firstInvalid-1); line <= lastInvalid; line++)
			{
                int y = textArea.lineToY(line) + fm.getHeight();
				paintOverline(gfx,line,x,y);
			}
            // end TIBCO change.
        ...
        // Start TIBCO change
        // restore translation from above.
        gfx.translate(-insets.left,-insets.top);
        // End tibco change.
	}
	...
	{
        // TIBCO change - make it include border area.
		repaint(getInsets().left,textArea.lineToY(line) + fm.getMaxDescent() + fm.getLeading()+getInsets().top,
	...
	{
        // TIBCO change - make it include border area
		repaint(getInsets().left,getInsets().top+textArea.lineToY(firstLine) + fm.getMaxDescent() + fm.getLeading(),
	...
	public float nextTabStop(float x, int tabOffset)
	{
        ...
        // TIBCO change
        int ltabSize = Math.max(1,tabSize); // just in case.
		int ntabs = ((int)x - offset) / ltabSize;
        // end TIBCO change
    ...
    // TIBCO change, added this:
    protected boolean eolMarkersForNewLinesOnly=true;
    // end TIBCO change.
	protected int cols;
	protected int rows;
    ...
    /**
     * TIBCO change.
     * Added to original jEdit source.  Allows painting over the line (i.e. error underlining) to happen in a subclass.<br>
     * Default implementation does nothing.
     * @param g The graphics
     * @param line The line number
     * @param x The x position of the line start
     * @param y The y position of the line start
     */
		...
		Color defaultColor, int x, int y)
	{
		...
		if(eolMarkers)
		{
            //start TIBCO CHANGE --- allow eol markers to be shown only on actual eols not just every line.
            if (!eolMarkersForNewLinesOnly || textArea.getLineCount()>line+1) {
    			gfx.setColor(eolMarkerColor);
                paintEOLMarker(gfx, x, y);
            }
            // end TIBCO change
		}
	}
    ...
    /**
     * TIBCO change -- added to allow override.
     */
