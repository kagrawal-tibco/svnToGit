...
/**
 * A (somewhat) generic formula text editor that's in the process of being factored out of being xpath specific.
 */
public class FormulaTextArea extends JSoftDragNDropablePathTextArea {
   /**
	 * 
	 */
...
private boolean mAllowsEmptyString;
   ...
   public FormulaTextArea(UIAgent uiAgent) {
   ...
   public void setSyntaxStyles(SyntaxStyle[] style) {
      if (style == null) {
         return;
      }
      SyntaxStyle[] existingStyles = getPainter().getStyles();

      for (int i = 0; i < style.length; i++) {
         SyntaxStyle ss = existingStyles[i];
         if (ss != null) {
            existingStyles[i] = ss;
         }
      }
      getPainter().setStyles(existingStyles);
   }
   ...
   private Utilities.DropTextInfo getDropTargetRange(int xPos, int yPos, int selStart, int selEnd, boolean isCharacterRef, boolean isSurround) {
      ...
      if (isSurround) {
         boolean canSurround = true; //WCETODO make this check if the dropped thing is a function.
         if (!canSurround) {
            return null;
         }
   ...
   private class MyDropHandler implements TextDragNDropHandler {
      ...
      public TextRange computeSurroundDropTextRange(Object dragObject, Point mouseAt) {
         ...
         if (sdi.path.indexOf("(") < 0) // cheap test for a function.
         {
   ...
   private class MyErrorFinder implements CodeErrorFinder {
      public ErrCheckErrorList getErrors(Reader documentReader) throws IOException {
         ...
         synchronized (FormulaTextArea.this) {
               ...
               DefaultCompletionProposal proposal = new DefaultCompletionProposal("Add namespace prefix  (Alt+Ent)", msg.getTextRange(), "") {
                  public void apply(Document document) {
                     ...
                     try {
                        ...
                        String replacementText = pfx + ":" + oqname.getLocalName();
