...
public class ClientDetailsPart implements IFormPart, IPartSelectionListener {
	...
	public static final String DETAILS_EMPTY_PAGE = "EmptyDetails";
 ...
 */	
	public ClientDetailsPart(IManagedForm mform, ScrolledPageBook pageBook) {
		this.pageBook = pageBook;
		pages = new Hashtable<Object, PageBag>();
		initialize(mform);
	}
/**
 ...
 */
	public ClientDetailsPart(IManagedForm mform, Composite parent, int style) {
 ...
 */
	...
	private void registerPage(Object objectClass, IDetailsPage page, boolean fixed) {
		...
		createEmptyDetailsInfo(managedForm.getToolkit(),pageBook.createPage(DETAILS_EMPTY_PAGE));
		pageBook.showPage(DETAILS_EMPTY_PAGE);
	}
/**
 ...
 */
	 ...
	 * @param composite
	 */
	private void createEmptyDetailsInfo(FormToolkit toolkit, Composite composite) {
		
		GridLayout layout = new GridLayout();
		layout.numColumns = 1;
//		layout.marginHeight = 120;
//		layout.marginWidth = 100;
		composite.setLayout(layout);
		
		Section deatailsSection = toolkit.createSection(composite, Section.EXPANDED | Section.TITLE_BAR);
		deatailsSection.setText("Details");
		deatailsSection.setActiveToggleColor(toolkit.getHyperlinkGroup().getActiveForeground());
		deatailsSection.setToggleColor(toolkit.getColors().getColor(IFormColors.SEPARATOR));
	
		deatailsSection.setFont(new org.eclipse.swt.graphics.Font(Display.getCurrent(), new FontData("Tahoma", 8, SWT.BOLD)));
		Composite sectionClient = toolkit.createComposite(deatailsSection, SWT.NONE);
		deatailsSection.setClient(sectionClient);
		layout = new GridLayout();
		sectionClient.setLayout(layout);
		deatailsSection.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		FormText txt = toolkit.createFormText(sectionClient, false);
		GridData gd = new GridData(GridData.FILL_HORIZONTAL);
		gd.verticalSpan = 50; 
		txt.setLayoutData(gd);
		txt.setText(Messages.getString("details.empty.message"), false, false);
