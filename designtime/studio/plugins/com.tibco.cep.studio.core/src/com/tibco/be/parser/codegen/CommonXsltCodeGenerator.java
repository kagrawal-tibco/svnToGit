...
public class CommonXsltCodeGenerator {
	...
	private class BindingContext {
		...
		public boolean isRuleFunctionCall() {
...
//			buffer.append(BRK);
	...
	private void processSetVariableBinding(SetVariableBinding binding,
		...
		String javaType = getJavaType(context.propertyType.type);
		if (javaType == null) {
			javaType = "Object";
		}
//		push declared var in a local var stack - could have multiple vars on same level - pop when parent binding finished processing
		...
		if (formula != null) {
        	String formulaString = processFormulaString(context.propertyType.type, context, formula, true);
...
//		}
		...
		if (context.isRuleFunctionCall() && context.binding.getChildCount() == 1 && context.binding.getChild(0).getChildCount() == 0) {
...
//			context.buffer.append("$currXiNode.setAtomicValue(com.tibco.xml.data.primitive.values.Xs").append(type).append("(").append(formulaString).append("));").append(BRK);
    	...
    	boolean isRFCall = context.isRuleFunctionCall();
    	boolean check = context.checkBinding;
    	String propertyType = null;
    	IVariableType type = this.typeResolver.getDeclaredIdentifierType(propName);
    	boolean isDeclaredSymbol = parent instanceof AttributeBinding && (context.isInputTransformation() || context.isPsuedoOutputTransformation()) && type != null;
    	if (isDeclaredSymbol) {
    		// this attribute is 'declared' in the symbol map - special case for input transformations
    		if (type.isConcept()) {
...
//		process element stack...
    	ListIterator<Entity> elements = context.sourceEntityStack.listIterator(1);
    	boolean hasElements = elements.hasNext();
    	boolean useAssertVar = hasElements && context.getAssertVarName() != null;
		if (!context.isInputTransformation() || (context.isInputTransformation() && context.sourceEntityStack.size() > 1)) {
		...
		if (parent instanceof AttributeBinding) {
    		...
    		if (!context.isInputTransformation() || (context.isInputTransformation() && hasElements)) {
    			setBuffer.append(".setPropertyValue(\"");
    			setBuffer.append(propName);
    			setBuffer.append("\", ");
...
//    			setBuffer.append(propName);
    	...
    	if (/*isRFCall && */context.isInputTransformation() && (parent instanceof AttributeBinding || isRFCall) || isProcessURIBinding(parent, propName)) {
    		requiresCast = false;
...
//    			setBuffer.append(")");
	...
	private String processFormulaString(String propertyType, BindingContext context, String formula, boolean processConstantFormula, String declVarName) {
		context.getCodegenContext().reset();
...
//				}
			...
			} else if (context.propertyType.isProperty && "com.tibco.cep.runtime.model.element.PropertyAtomConcept".equals(forLoopInfo.getLocalVarType())) {
				if (isConstantFormula(chars) && ModelNameUtil.isValidIdentifier(formula)) {
					newFormula.append(forLoopInfo.getLocalVarName());
					newFormula.append('.');
...
//				return formula; // ?
				if (processConstantFormula) {
					if (!context.isInputTransformation() && !context.isRuleFunctionCall() && !context.isPsuedoOutputTransformation()) {
...
//    					buf.append((char)b);
				...
				if (b == '/') {
					...
					currentVar = "$"+variable;
					if ("$3zglobalVariables".equals(currentVar)) {
