...
//        }
    ...
    private boolean contains(LineInfo li) {
        for(int i=0;i < lineData.size();i++) {
            LineInfo line = (LineInfo) lineData.get(i);
            if(
                    line.inputStartLine == li.inputStartLine &&
                    line.outputStartLine == li.outputStartLine &&
                    (
                       line.inputLineCount > li.inputLineCount ||
                       line.outputLineIncrement > li.outputLineIncrement
                    )
              )
            {
                return true;    
    ...
    public List<LineInfo> getLineData() {
        Map lineMap = new LinkedHashMap();
        for(Iterator it = lineData.iterator();it.hasNext();) {
            LineInfo li = (LineInfo) it.next();
            if(!lineMap.containsKey(li.getXmlString())) {
                lineMap.put(li.getXmlString(),li);
            }
        }
        return new ArrayList<LineInfo>(lineMap.values());
    }
