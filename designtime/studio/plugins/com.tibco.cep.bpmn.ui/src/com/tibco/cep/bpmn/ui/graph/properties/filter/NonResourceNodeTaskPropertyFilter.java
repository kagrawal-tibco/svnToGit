 ...
 * 
 */
public class NonResourceNodeTaskPropertyFilter implements IFilter {
	/*
	 * (non-Javadoc)
	 * 
	...
	public boolean select(Object toTest) {
		...
		if (toTest instanceof TSENode) {
			...
			if (tsg.getUserObject() != null && tsg.getUserObject() instanceof EObject) {
				EObjectWrapper<EClass, EObject> userObjWrapper = EObjectWrapper.wrap((EObject) tsg.getUserObject());
				if (BpmnModelClass.TASK.isSuperTypeOf(userObjWrapper.getEClassType())
						&& !BpmnModelClass.INFERENCE_TASK.isSuperTypeOf(userObjWrapper.getEClassType())) {
