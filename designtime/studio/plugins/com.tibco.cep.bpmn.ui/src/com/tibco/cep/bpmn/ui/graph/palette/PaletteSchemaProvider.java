...
import com.tibco.cep.studio.common.palette.util.CommonPaletteUtils;
 ...
 *
 */
public class PaletteSchemaProvider implements SmNamespaceProvider,
		SchemaModelHandler, SchemaModelProvider {

	private final HashMap<String, SmNamespace> namespaces;

	public PaletteSchemaProvider() throws SAXException, IOException {
		namespaces = new HashMap<String, SmNamespace>();

        InputSource is;
        URL schemaUrl = CommonPaletteUtils.getPaletteSchemaURL();
        if (null != schemaUrl) {
        	InputStream openStream = schemaUrl.openStream();
            is = new InputSource(openStream);
        } else {
        	InputStream resourceAsStream = CommonPaletteUtils.getPaletteResAsStream();
            is = new InputSource(resourceAsStream);
            schemaUrl = BpmnUIPlugin.class.getProtectionDomain().getCodeSource().getLocation();
        }
	...
	@Override
	public Iterator<SmNamespace> getNamespaces() {
		return namespaces.values().iterator();
	}
	
	@Override
	public SmNamespace getNamespace(String namespaceURI) {
        BpmnUIPlugin.debug("URI: " + namespaceURI);
