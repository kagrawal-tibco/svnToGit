...
public class IconSelectionDialog extends Dialog {
	...
	protected Button bInsideProject, bFileSys, bOk, bCancel, imageDisplay;
	protected Canvas canvas;
	protected Text filePath;
	protected Label empty;
	protected Table iconTable;
	protected ArrayList<String> filter = new ArrayList<String>();
	protected String fileName= new String();
	protected Set<String> iconSet=new HashSet<String>();
	protected HashSet<String> newIcons=new HashSet<String>();
	private Image sourceImage;
	private Image screenImage;
	private AffineTransform transform = new AffineTransform();
	private static final int MAX_LENGTH_ITEM = 6;
	public String selectedImageData=null;
	
	/**
	 * @param parent
	 * @param mdlmgr {@link BpmnPaletteConfigurationModelMgr}
	 */
	public IconSelectionDialog(Shell parent, BpmnPaletteConfigurationModelMgr mdlmgr) {
		super(parent);
	...
	protected Control createContents(Composite parent) {
		...
		GridLayoutFactory.fillDefaults().margins(LayoutConstants.getMargins())
				...
				defaultSpacing.y).numColumns(1).applyTo(parent);
	...
	protected Control createDialogArea(Composite parent) {
		...
		Composite mainCompsite = new Composite(parent, SWT.NONE);
		GridData gd = new GridData(GridData.FILL_HORIZONTAL);
		mainCompsite.setLayoutData(gd);
		mainCompsite.setLayout(new GridLayout(2, false));
		
		filter.add("*.gif");
		filter.add("*.png");
		
		filePath = PanelUiUtil.createText(mainCompsite, 250);

/*
		bInsideProject = new Button(mainCompsite, SWT.PUSH);
		bInsideProject.setText("Icon");
		gd = new GridData(GridData.HORIZONTAL_ALIGN_BEGINNING);
		gd.widthHint = 150;
		bInsideProject.setLayoutData(gd);
		bInsideProject.setEnabled(true);

		empty=PanelUiUtil.createLabel(mainCompsite, "");*/

		bFileSys = new Button(mainCompsite, SWT.PUSH);
		...
		createIconTable(mainCompsite);

		filltable();

		gd = new GridData(GridData.HORIZONTAL_ALIGN_BEGINNING);
		gd.widthHint = 200;
		iconTable.setLayoutData(gd);
		iconTable.setEnabled(true);

		createCanvasComponent(mainCompsite);
	...
	private void createIconTable(Composite parent) {
		...
		GridData data = new GridData(GridData.HORIZONTAL_ALIGN_BEGINNING | GridData.FILL_BOTH);
		data.heightHint = 100;
		data.widthHint = 100;
	...
	private void createIconTableEditor(final TableItem item) {
		...
		for (int i = 0; i < MAX_LENGTH_ITEM ; i++ ) {
			...
			Image image = BpmnImages.getInstance().getImage(iconPath);
			item.setText(i, "");
	...
	protected void okPressed() {
		 selectedImageData = (String) canvas.getData();
