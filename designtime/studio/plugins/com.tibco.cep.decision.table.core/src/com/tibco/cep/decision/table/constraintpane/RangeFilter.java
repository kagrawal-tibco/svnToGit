...
/**
 * Created by IntelliJ IDEA. User: suresh Date: Aug 3, 2008 Time: 1:14:09 AM To
...
public class RangeFilter implements Filter {
	...
	public void updateCell(List<Cell> oldCells, List<Cell> updatedCells) {
		DecisionTableCorePlugin.debug(CLASS, "Updating cell(s) {0}",
				oldCells.toArray());
	...
	public void removeCell(Cell removeCell) {
		...
		if (expr1 instanceof Integer) {
			...
			try {
				...
				DecisionTableCorePlugin.log(nfe);
			}
		}
	...
	private void removeCell(long expr, Cell cellToRemove) {
		LinkedHashSet<Cell> cells = rangeMap.get(expr);
		if (null == cells) {
