...
//		showDescription.setText(Messages.getString("DECISION_TABLE_SHOW_DOMAIN_DESCRIPTION"));
			...
			try{
				...
				if ((checkValidAccess(virtualRuleFunctionPath)) && super.validatePage() && (isImportWizard)) {
					...
					if (((fullFileName.endsWith(".xls"))) && (dtname) && (vrfname) && (parentFolderName) 
							&& (CommonUtil.isValidName(dtNameText.trim())) && !DT_NAME_CHANGE && isValidDTName(dtNameText)) {
	...
	public boolean isValidDTName(String dtName){
		if(template.getName().trim().equalsIgnoreCase(dtName.trim())){
			return false;
		}else{
			return true;
		}	
	...
	protected boolean isDuplicateBEResource(IResource resource,
		...
		// check in Project Library if any
		if (!result) result = DecisionTableCoreUtil.isDuplicateDTInProjectLibrary(containerResource.getProject().getName(), virtualRFText.getText().toString(),"/" + containerResource.getProjectRelativePath() + "/" + containerResourceName, duplicateFileName);
