...
public class ThreadFilterEditor {
	...
	private RuleBreakpointAdvancedPage fPage;
	...
	public ThreadFilterEditor(Composite parent, RuleBreakpointAdvancedPage page) {
		fPage= page;
	...
	private void createThreadViewer(Composite parent) {
		...
		label.setText("&Restrict to Selected Thread(s):"); 
	...
	protected void setInitialCheckedState() {
		IDebugTarget[] targets= getDebugTargets();
		for (int i= 0, numTargets= targets.length; i < numTargets; i++) {
			IRuleRunTarget target = (IRuleRunTarget)targets[i].getAdapter(IRuleRunTarget.class);
			if (target != null) {
				IRuleDebugThread filteredThread;
				try {
					filteredThread = fPage.getBreakpoint().getThreadFilter(target);
		...
		protected void checkTarget(IDebugTarget target, boolean checked) {
			...
			if (checked) {
				...
				try {
					...
					StudioDebugUIPlugin.log(exception);
					return;
				}
				IThread thread;
		...
		protected void verifyCheckedState() {
			...
			for (int i= 0, numTargets= targets.length; i < numTargets; i++) {
				...
				if (checkedThread) {
					...
					fPage.setErrorMessage("Must select a thread in selected targets"); 
				}
			}
		}
	...
	class ThreadFilterContentProvider implements ITreeContentProvider {
		...
		public Object[] getChildren(Object parent) {
			if (parent instanceof IDebugTarget) {
				IRuleDebugTarget target = (IRuleDebugTarget)((IDebugTarget)parent).getAdapter(IRuleDebugTarget.class);
				if (target != null) {
					try {
						return ((IRuleDebugTarget)parent).getThreads();
			...
			if (parent instanceof ILaunchManager) {
				...
				for (int i= 0, numLaunches= launches.length; i < numLaunches; i++) {
					...
					for (int j= 0, numTargets= targets.length; j < numTargets; j++) {
						target= (IRuleDebugTarget)targets[j].getAdapter(IRuleDebugTarget.class);
