...
import com.tibco.cep.studio.decision.table.constraintpane.DecisionTableCreator;
			...
			for (int loop = checkpointDeltaCommands.size() - 1; loop >= 0; loop--) {
				...
				if (command instanceof IExecutableCommand) {
					...
					try {
						...
						// updating constraints table should not prevent source thread from continuing
