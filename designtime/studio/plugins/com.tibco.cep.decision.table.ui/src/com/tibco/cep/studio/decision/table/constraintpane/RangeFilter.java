...
public class RangeFilter implements Filter {
	...
	public void updateCell(List<Cell> oldCells, List<Cell> updatedCells) {
		DecisionTableUIPlugin.debug(CLASS, "Updating cell(s) {0}",
				oldCells.toArray());
	...
	public void removeCell(Cell removeCell) {
		...
		if (expr1 instanceof Integer) {
			...
			if (removeCell.getCellInfo().getType() == PROPERTY_TYPES.DOUBLE_VALUE) {
				removeCell(((Long) expr1).doubleValue(), removeCell);
			} else {
				removeCell((Long) expr1, removeCell);
			}
		}else if(expr1 instanceof Double){
			removeCell((Double) expr1, removeCell);
		...
		if (expr2 instanceof Integer) {
			...
			if (removeCell.getCellInfo().getType() == PROPERTY_TYPES.DOUBLE_VALUE) {
				removeCell(((Long) expr2).doubleValue(), removeCell);
			} else {
				removeCell((Long) expr2, removeCell);
			}
		}else if(expr2 instanceof Double){
			removeCell((Double) expr2, removeCell);
	...
	private void _processRemoved(Cell removeCell, Object expr1) {
	...
	public void addCell(Cell c) {
		...
		if (c.getCellInfo().getType() == PROPERTY_TYPES.INTEGER_VALUE
				|| c.getCellInfo().getType() == PROPERTY_TYPES.LONG_VALUE || c.getCellInfo().getType() == PROPERTY_TYPES.DOUBLE_VALUE) {
	...
	public Object[] getMinMax(boolean excludeIntegerMinMax) {
		...
		}else if(minmax[0] instanceof Double){
			if ((Double) minmax[0] == Double.MIN_VALUE)
				minmax[0] = rangeArray[1];
		}
		if (minmax[1] instanceof Long) {
			if ((Long) minmax[1] == Long.MAX_VALUE)
				minmax[1] = rangeArray[rangeArray.length - 2];
		}else if(minmax[1] instanceof Double){
			if ((Double) minmax[1] == Double.MAX_VALUE)
				minmax[1] = rangeArray[rangeArray.length - 2];
		}
		try {
			...
			if (minmax[0] instanceof Date) {
				if (minDate.equals((Date)minmax[0]) && rangeArray.length > 2)
					minmax[0] = rangeArray[1];
			}
			if (minmax[1] instanceof Date) {
				if (maxDate.equals((Date)minmax[1]) && rangeArray.length > 2)
					minmax[1] = rangeArray[rangeArray.length - 2];
			}
