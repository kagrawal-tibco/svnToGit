...
import static com.tibco.cep.studio.decision.table.constraintpane.DecisionTableAnalyzerUtils.getConfiguredDomains;
import static com.tibco.cep.studio.decision.table.constraintpane.DecisionTableAnalyzerUtils.getDomainValues;
import static com.tibco.cep.studio.decision.table.constraintpane.DecisionTableAnalyzerUtils.getPropertyPathForColumn;
...
public class AnalyzeTableRunnable {
	...
	protected AnalysisProblemsCollector problemsCollection = new AnalysisProblemsCollector();

	protected IFile decisionTableFile;

	/**
	 * If true domain models associated with properties of the table will be
	 * used for table completeness.
	 */
	private boolean useDomainModel;

	private IDecisionTableAnalyzerExtension formatter;

	public AnalyzeTableRunnable(DecisionTable optimizedTable, Table table,
			boolean useDomainModel, IDecisionTableAnalyzerExtension formatter) {
		this.fOptimizedTable = optimizedTable;
		this.fEMFTable = table;
		this.useDomainModel = useDomainModel;
		this.formatter = formatter;
		this.decisionTableFile = formatter != null ? formatter
				.getUnderlyingFile() : null;
	}

	protected void addMarkers(final IResource resource, SubMonitor monitor)
			throws Exception {
		if (resource == null) {
			return;
		}
		monitor.setTaskName("Deleting previous markers");
		resource.deleteMarkers(ANALYZE_FIXABLE_MARKER_NAME, true,
		...
		try {
			...
			DecisionTableUIPlugin.debug(CLASS,
					"Started marker generation at {0}",
			...
			DecisionTableUIPlugin.debug(CLASS,
					"Finished marker generation at {0}",
					System.currentTimeMillis());
	...
	public void analyze(SubMonitor monitor) throws InvocationTargetException,
		...
		monitor.beginTask("Analyzing table " + getTable().getName(),
		...
		collectAllActions(table, gatheringActionsMonitor, allActions,
				allRuleCells);
		// monitor.worked(30);
		...
		analyzeRulesForConflictingActions(table,
				conflictingActionsCheckMonitor, allActions, allRuleCells);
		...
		analyzeRulesToBeCombined(table, ruleCombiningCheckMonitor, allActions,
				allRuleCells);
		monitor.worked(85);
		if (monitor.isCanceled()) {
			return;
	 ...
	 *            -> All conditions
	 */
	private void collectAllActions(DecisionTable table, SubMonitor monitor,
		...
		for (String column : columns) {
			...
			for (Filter filter : allFilters) {
				collectActions(table, column, filter, monitor, allActions,
						allRuleCells);
			}
	...
	private void analyzeRulesForConflictingActions(DecisionTable table,
			SubMonitor monitor, Map<String, List<Cell>> allActions,
			Map<String, List<Cell>> allRuleCells) {
				...
				if (areEqual) {
					...
					if (!equalActions) {
						String message = Messages
								.getString(
										"DecisionTable.analyze.conflictingActions.message",
										lhsId, rhsId);
						problemsCollection.reportConflictingActionsProblem(
								decisionTableFile, message,
		...
		Date end = new Date();
		System.out.println("Rule conflict change check took "
	...
	private void analyzeRulesToBeCombined(DecisionTable table,
		...
		for (int i = 0; i < size; i++) {
			...
			List<String> equalRules = getEqualRules(ids, allActions, list,
					i + 1);
			// Get duplicate rules
			for (String id2 : equalRules) {
				String message = Messages.getString(
						"DecisionTable.analyze.equalRules.message", id, id2);
				...
				problemsCollection.reportRuleCombinationProblem(
						decisionTableFile, message, id2,
						IMarker.SEVERITY_WARNING);
			}
		}
		Date end = new Date();
		DecisionTableUIPlugin.debug(CLASS, "Rule combination check took {0}",
				(end.getTime() - start.getTime()));
	}
	...
	private void reportCellCombination(Cell cell, Cell cell2) {
		DecisionTableUIPlugin.debug(CLASS,
				"conditions can be combined: {0} and {1}", cell.getRuleTupleInfo().getId(),
				cell2.getRuleTupleInfo().getId());
		...
		// this will already get reported by the analyzeRulesToBeCombined method
//		problemsCollection
//				.reportProblem(decisionTableFile, message.toString(), details.toString(),
//						Integer.parseInt(cell2.getRuleTupleInfo().getId()), -1, -1,
//						IMarker.SEVERITY_WARNING);
	}
	...
	private List<String> getEqualRules(Object[] ids,
			Map<String, List<Cell>> allActions, List<Cell> list, int i) {
	...
	private void collectActions(DecisionTable table, String columnName,
			Filter filter, SubMonitor monitor,
			Map<String, List<Cell>> allActions,
			Map<String, List<Cell>> allRuleCells) {
		...
		for (Set<Cell> set : values) {
			...
			for (Cell cell : set) {
				...
				if (allRuleCells.containsKey(cell.getRuleTupleInfo().getId())) {
					List<Cell> cells = allRuleCells.get(cell.getRuleTupleInfo().getId());
	...
	private void analyzeTableCompleteness(DecisionTable table,
			SubMonitor monitor) {
		if (table == null) {
			return;
		}
		...
		for (String column : columns) {
			...
			for (Filter filter : allFilters) {
				if (filter instanceof RangeFilter) {
					...
					if (useDomainModel) {
						// Get property path
						String propertyPath = getPropertyPathForColumn(column,
								table);
						List<DomainInstance> domainInstances = getConfiguredDomains(
								propertyPath, projectName);
						if (domainInstances.size() > 0) {
							values.addAll(rangeFilter.getRangeMap().values());
						} else {
							// Go the regular route
							analyzeRangeCompleteness(column,
									(RangeFilter) filter, monitor);
						}
					} else {
						analyzeRangeCompleteness(column, (RangeFilter) filter,
								monitor);
					}
				}
				if (filter instanceof EqualsFilter) {
					if (useDomainModel) {
						/**
						 * Analyze coverage for equals based on associated
						 * domain model. This is called only if domain model
						 * flag is true
						 */
			...
			if (useDomainModel) {
				...
				// Retrieve the cells if values is empty
				if (values.isEmpty()) {
					List<LinkedHashSet<Cell>> cells = table.getCells(column);
					values.addAll(cells);
				}
				analyzeDomainEqualsCoverage(values, propertyPath, column,
						projectName);
	...
	private void analyzeEqualsCompleteness(String columnName, SubMonitor monitor) {
		...
		RangeFilter columnRangeFilter = fOptimizedTable
				.getRangeFilter(columnName);
		EqualsFilter columnEqualsFilter = fOptimizedTable
	...
	/**
	 * The cell values collection contains all conditions for that particular
	 * column.
	 * 
	 * @param cellValues
	 * @param propertyPath
	 * @param columnName
	 *            -> The actual column name required to report the problem
	 * @param project
	 */
	
	private void analyzeDomainEqualsCoverage(
			Collection<LinkedHashSet<Cell>> cellValues, String propertyPath,
			String columnName, String project) {
		IPreferenceStore prefStore = DecisionTableUIPlugin.getDefault().getPreferenceStore();
		String[] domainValues = getDomainValues(propertyPath, project, "",prefStore.getBoolean(PreferenceConstants.SHOW_DOMAIN_DESCRIPTION));
		if (domainValues != null && domainValues.length > 0) {
			...
			System.arraycopy(domainValues, 1, newDomainValues, 0,
					domainValues.length - 1);

			// No need to analyze equals coverage
			outer: for (String domainValue : newDomainValues) {
				...
				for (Set<Cell> values : cellValues) {
					for (Cell value : values) {
						...
						// FIX BE-14268: Don't care was being compared with
						// other Values and was unequal.So Ignore Don't Care.
				...
				String message = Messages.getString(
						"DecisionTable.analyze.uncoveredDomainEntry.message",
						domainValue, propertyPath);
				problemsCollection.reportUncoveredDomainEntriesProblem(
						decisionTableFile, message, domainValue, columnName,
						IMarker.SEVERITY_WARNING);
			}
		}
	}

	private void analyzeRangeCompleteness(String columnName,
			RangeFilter filter, SubMonitor monitor) {
		...
		Date end = new Date();
		DecisionTableUIPlugin.debug(CLASS, "Covered range check took {0}",
				(end.getTime() - start.getTime()));
		start = new Date();
		...
		DecisionTableUIPlugin.debug(CLASS, "Overlapping range check took {0}",
				(end.getTime() - start.getTime()));
	}

	private void analyzeOverlappingRanges(List<Range> allRanges,
			SubMonitor monitor) {
	...
	private void reportMissingEqualsCriterion(String columnName,
		...
		String message = Messages.getString(
				"DecisionTable.analyze.missingEqualsCriterion.message",
				columnName, expressionKey);
		problemsCollection.reportUncoveredEqualsProblem(decisionTableFile,
				message, expressionKey, columnName, IMarker.SEVERITY_WARNING);
	}
	...
	private void reportOverlappingRangeError(Range range,
		...
		problemsCollection.reportOverlappingRangeProblem(decisionTableFile,
				details.toString(), Integer.parseInt(ruleId),
				IMarker.SEVERITY_WARNING);
	}
	...
	private void analyzeCoveredRanges(List<Range> coveredRanges,
		...
		for (int i = 0; i < rangeSize; i++) {
			...
			if (range.cell.getExpression().getOperandRangeKinds()
					.equals(Operator.RANGE_BOUNDED)
					|| range.cell.getExpression().getOperandRangeKinds()
							.equals(Operator.RANGE_MIN_EXCL_BOUNDED)) {
				rangeChars = new char[] { '[', ']' };
			} else {
				rangeChars = new char[] { '[', ')' };
			...
			if (range.min instanceof Date && range.max instanceof Date) {
				try {
					...
					if (((Date) min).before((Date)range.min)) {// &&
						...
						// range.cell.expression.rangeKind !=
						// Operator.RANGE_MAX_EXCL_BOUNDED)) {
						if (((Date) min).equals(format.parse("1601-01-01T00:00:00"))) {
							reportUncoveredUnboundedRange(range.columnName,
									rangeChars, range.min, tablePath, ruleId,
									columnId, false);
						} else {
					...
					if (i == rangeSize - 1) {
						...
						if (((Date) max).after((Date)range.max)) {
							if (((Date) max).equals(format.parse("9999-12-31T00:00:00"))) {
								reportUncoveredUnboundedRange(range.columnName,
										rangeChars, range.max, tablePath, ruleId,
										columnId, true);
							} else {
	...
	private void reportUncoveredBoundedRange(String columnName,
			char[] rangeChars, Object max2, Object min, String tablePath,
			String ruleId, String columnId) {
		// String message="";

		// if ()
		String message = Messages.getString(
				"DecisionTable.analyze.uncoveredBoundedRange.message",
				columnName, rangeChars[0], max2, min, rangeChars[1]);
		...
		// reportProblem(decisionTableFile, message.toString(),
		// Integer.parseInt(ruleId), -1, -1, IMarker.SEVERITY_WARNING);
		problemsCollection.reportRangeProblem(decisionTableFile,
				message.toString(), columnName, max2, min,
				RANGE_TYPE_PROBLEM.RANGE_BETWEEN_PROBLEM,
				Integer.parseInt(ruleId), IMarker.SEVERITY_WARNING);
	}

	private void reportUncoveredUnboundedRange(String columnName,
			char[] rangeChars, Object min, String tablePath, String ruleId,
		...
		// reportProblem(decisionTableFile, message.toString(),
		// Integer.parseInt(ruleId), -1, -1, IMarker.SEVERITY_WARNING);
		problemsCollection.reportRangeProblem(decisionTableFile, message,
				columnName, min, min, rangeTypeProblem,
				Integer.parseInt(ruleId), IMarker.SEVERITY_WARNING);
	}
