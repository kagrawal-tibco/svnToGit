...
public class DecisionTable {
	...
	private static final String CLASS = DecisionTableUIPlugin.class.getName();
    ...
    public void addCell(Cell c) {
    	DecisionTableUIPlugin.debug(CLASS, "Adding Cell {0}", c);
        if (c != null) {
            if (c.ci != null && c.ci.alias != null) {
                LinkedList<Cell> cells = conditionMatrix.get(c.ci.alias);
                if (cells == null) {
    ...
    public void toggleRule(DecisionRule rule){
    	Cell refCell = null;
    	for (List<Cell> aliasCells : conditionMatrix.values()) {
			if (!aliasCells.isEmpty()) {
				for (Cell cell : aliasCells) {
					String condRuleId = DecisionTableUtil.getContainingRuleId(cell.getId());
					if (condRuleId.equals(Integer.toString(rule.getId()))) {
						cell.setEnabled(rule.isEnabled());
						refCell = cell;
    	...
    	RuleTupleInfo ruleTupleInfo = idToRuleTuples.get(Integer.toString(rule.getId()));
    	for (Cell cell : ruleTupleInfo.getActions()){
    		cell.setEnabled(rule.isEnabled());
    	}
    }
    ...
    public void updateCell(String alias, 
    	...
    	DecisionTableUIPlugin.debug(CLASS, "Caling Update Cell for Cell Id {0} and expression {1}", cellId, expression);
        		...
        		if (ruleId != null) {
        			...
        			if (ruleTupleInfo != null) {
	    		    	...
	    		    	while (tokens.hasMoreTokens()) {
	    		    		...
	    		    		Cell newCell = new Cell(refCell.cellType);
	    		    		newCell.ri = refCell.ri;
	    		    		newCell.ci = refCell.ci;
	    		    		newCell.columnId = refCell.columnId;
	    		    		newCell.id = refCell.id;
    ...
    public void removeCell(Cell cell) {
    	 if (cell != null) {
			if (cell.ci != null && cell.ci.alias != null) {
				byte cellType = cell.cellType;
				switch (cellType) {
				case 0:
					List<Cell> cells = conditionMatrix.get(cell.ci.alias);
					if (cells != null) {
						cells.remove(cell);
    ...
    public void printStats() {
        ...
        for (Map.Entry<String, LinkedList<Cell>> entry : entrySet) {
            ...
            for (Cell c : lc) {
            	...
            	if (expression != null) {
            		++(cellTypes[expression.operatorKind]);
            	}
            }
        }

        DecisionTableUIPlugin.debug("******** Non Optimized ***** ");
        DecisionTableUIPlugin.debug(CLASS, "#Rules, #Conditions = [{0}, {1}]",  nosOfRules , nosOfConditions );
        DecisionTableUIPlugin.debug(CLASS, "#Total Cell size = {0} MB",  ((nosOfConditions * 120)/1000000));
        ...
        DecisionTableUIPlugin.debug("******** Optimized ***** ");
        DecisionTableUIPlugin.debug(CLASS, "#Rules, #Conditions = [{0}, {1}]",  nosOfRules , nosOfConditions );
        for (int i = 0; i < cellTypes.length; i++) {
        	//LOGGER.logInfo("# Conditions[%1$s]=%2$s", Operator.OperatorTypes[i] , cellTypes[i]);
        }
    ...
    public void optimize(SubMonitor subMonitor) {
    	...
    	for (Map.Entry<String, LinkedList<Cell>> entry : entrySet) {
    		...
    		for (Cell c : lc) {
    			if (c.expression == null) { // remove nulls
    				continue;
    			}
    			Filter filter = filters[c.expression.operatorKind];
    	...
    	DecisionTableUIPlugin.debug(CLASS, "Total time for optimization :{0}(ms)", (endTime - startTime)/1000000);
    ...
    public void executeOptimized(HashMap input) throws Exception {
        ...
        DecisionTableUIPlugin.debug(getClass().getName(), "#Nos of rules to be fired - {0}", resultantRules.size());
        DecisionTableUIPlugin.debug(getClass().getName(), "The following rules have to be fired : {0}" , resultantRules.toString() );
    }
...
//    	List<Filter> filters = getAllFilters(columnName);
    	...
    	if (filters == null) {
        	...
        	Filter filter = filterArray[cell.expression.operatorKind];
            if (filter != null) { 
            	filter.addCell(cell); 
            }
			...
			for (Filter f : filters) {
				if (f instanceof EqualsFilter) {
					...
					if (filter.getKeySet().contains(expr)) {
						cells = new ArrayList<Cell>(filter.map.get(expr));
					}
				}
			}
		}
		...
		if (filters != null) {
			for (Filter f : filters) {
				if (f instanceof RangeFilter) {
					...
					if (filter.rangeMap.containsKey(expr)) {
						cells = new ArrayList<Cell>(filter.rangeMap.get(expr));
					}
				}
			}
		}
