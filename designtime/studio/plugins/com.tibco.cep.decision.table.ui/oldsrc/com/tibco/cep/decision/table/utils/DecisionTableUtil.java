...
import static com.jidesoft.decision.cell.editors.utils.CellEditorUtils.massageRawString;
...
public class DecisionTableUtil {
	...
	public static StandardDialog fieldSettingsDialog = null;	
	
	public static StandardDialog commentsDialog = null;	
	
	public static IMenuManager manager = null;

	public static JidePopup actionpopup = null;
	
	public static JidePopup custompopup = null;
...
//					table.getName());
	 ...
	 * @param isWizardOnTop
	 */
	public static void hideAllEditorPopups(final IWorkbenchPart part, final boolean isWizardOnTop) {
		if (!isWizardOnTop) {
			IsEditorPopupMenuEnabled = false;
		}
		...
		/*SwingUtilities.invokeLater(new Runnable() {
			...
			@Override
			public void run() {
				if (commentsDialog != null && commentsDialog.isVisible()) {
					commentsDialog.setVisible(false);
				}
				
				if (fieldSettingsDialog != null && fieldSettingsDialog.isVisible()) {
					fieldSettingsDialog.setVisible(false);
				}
				
				if (actionpopup != null && actionpopup.isPopupVisible()) {
					actionpopup.hidePopupImmediately();
				}
				
				if (custompopup != null && custompopup.isPopupVisible()) {
					custompopup.hidePopupImmediately();
				}
				stopCellEditing(part);
			}
		});*/
		if (commentsDialog != null && commentsDialog.isVisible()) {
			commentsDialog.setVisible(false);
	 ...
	 * @param part
	 */
	private static void stopCellEditing(IWorkbenchPart part) {
		if (part instanceof DecisionTableEditor) {
			final DecisionTableEditor editor = (DecisionTableEditor) part;
			DecisionTableDesignViewer decDesignViewer = editor.getDesignViewer();
			if (decDesignViewer != null) {
				decDesignViewer.stopPopupCellEditing(decDesignViewer.getDecisionTablePane());
				decDesignViewer.stopPopupCellEditing(decDesignViewer.getExceptionTablePane());
			}
		}
	}
...
//				}
	...
	public static boolean isVarString (String value, int areaType) {
		boolean bool = false;
		if (DecisionField.AREA_CONDITION == areaType) {
			bool = isConditionVarString(value);
		} else if (DecisionField.AREA_ACTION == areaType) {
	...
	public static void updateAutoMerge(boolean autoMerge) {
		...
		for (IEditorReference ref : references) {
			if (ref.getEditor(false) instanceof DecisionTableEditor) {
				...
				DecisionTablePane pane = viewer.getDecisionTablePane();
				JTable mainTable = pane.getTableScrollPane().getMainTable();
				JTable headerTable = pane.getTableScrollPane().getRowHeaderTable();
	            int cellMerge = (autoMerge == true) ? CellSpanTable.AUTO_CELL_MERGE_ROWS_LIMITED 
	 ...
	 * 
	 */
	public static void updateColumnFilterVisibility() {
		IEditorReference[] references = PlatformUI.getWorkbench().getActiveWorkbenchWindow()
    												.getActivePage().getEditorReferences();
		for (IEditorReference ref : references) {
			if (ref.getEditor(false) instanceof DecisionTableEditor) {
				...
				decisionTablePane.updateFilterButtons();
				DecisionTablePane exceptionTablePane = viewer.getExceptionTablePane();
				exceptionTablePane.updateFilterButtons();
			}
		}
	}
	...
	public static String getAliasFromSubstitution(String alias) {
		...
		boolean isSubstitution = (isVarString(alias, DecisionField.AREA_ACTION) 
				|| isVarString(alias, DecisionField.AREA_CONDITION));
