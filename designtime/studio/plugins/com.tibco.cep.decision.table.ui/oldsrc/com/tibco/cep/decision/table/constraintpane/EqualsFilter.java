...
public class EqualsFilter implements Filter {
	...
	// Eg. 10=>[C12, C22, C32, ...]
    HashMap<Object, LinkedHashSet<Cell>> map = new HashMap<Object, LinkedHashSet<Cell>>();
    ...
    public void addCell(Cell c) {
    	if (!checkOperator(c.expression.operatorKind)) {
    		return;    	
    	}
    	Object equalExpr = c.expression.operands[0];
    ...
    public void updateCell(List<Cell> oldCells, List<Cell> updatedCells) {
		// clear old cells
    	DecisionTableUIPlugin.debug(this.getClass().getName(), "Updating cells {0}", oldCells.toArray());
		for (Cell oldCell : oldCells) {
			// skip range cells
			if (!checkOperator(oldCell.expression.operatorKind)) {
	    		return;    	
	    	}
			
			Object equalExpr = oldCell.expression.operands[0];
			LinkedHashSet<Cell> cells = map.get(equalExpr);
			if (cells != null) {
				if (cells.size() == 1) {
		...
		for (Cell updatedCell : updatedCells) {
			...
			// skip range cells
			if (!checkOperator(updatedCell.expression.operatorKind)) {
	    		return;    	
	    	}
			
			Object updatedExpression = updatedCell.expression.operands[0];
	...
	public void removeCell(Cell removeCell) {
		if (!checkOperator(removeCell.expression.operatorKind)) {
    		return;    	
    	}
		Object equalExpr = removeCell.expression.operands[0];
		LinkedHashSet<Cell> cells = map.get(equalExpr);
		if (null == cells) {
			// Clear the entry
			map.remove(equalExpr);
