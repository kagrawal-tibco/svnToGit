...
import com.jidesoft.decision.IDGenerator;
...
public class RuleIDGeneratorManager {
	...
	private static IDGenerator<String> getIDGenerator(String project, String tablePath) {
	 ...
	 * @param decisionRule
	 */
	public void setID(String project, String tablePath, DecisionRule decisionRule) {
		IDGenerator<String> idGenerator = getIDGenerator(project, tablePath);
		Integer newId = Integer.parseInt(idGenerator.getNextID());
		decisionRule.setId(newId);
		for (DecisionEntry condition : decisionRule.getConditions()) {
		...
		for (DecisionEntry action : decisionRule.getActions()) {
