...
import static com.tibco.cep.studio.wizard.ftl.FTLWizardConstants.UNDERSCORE;
...
public class EntityCreator {
	public static SimpleEvent createEvent(String projectName, String eventFolder, String eventName, Properties eventProps, Channel channel, String destName) {
		SimpleEvent event = EventFactory.eINSTANCE.createSimpleEvent();
		populateEntity(projectName, eventName, "/" + eventFolder + "/", event);
		if(destName != null) {
		   event.setChannelURI(channel.getFullPath());
		   event.setDestinationName(destName);
		}
		...
		Rule expiryAction = RuleFactory.eINSTANCE.createRule();
		expiryAction.setName(eventName + "_expiryAction");
		expiryAction.setOwnerProjectName(projectName);
		expiryAction.setActionText("");
		expiryAction.setConditionText("");
		
		Symbols symbols = RuleFactory.eINSTANCE.createSymbols();
		EList<Symbol> symbolEList = symbols.getSymbolList();
		Symbol symbol = RuleFactory.eINSTANCE.createSymbol();
		symbol.setIdName(eventName);
		symbol.setType("/" + eventFolder + "/" + eventName);
		symbolEList.add(symbol);
		expiryAction.setSymbols(symbols);
		
		event.setExpiryAction(expiryAction);
		...
		if (eventProps != null) {
			for (int i = 0; i < eventProps.keySet().size(); i++) {
				...
				if ("message".equals(type)||"inbox".equals(type)||
						"opaque".equals(type)||type.contains("array")){
					continue;
				}
				
				if (propName.contains("-")) {
					propName = propName.replace("-",
							FTLWizardConstants.ESCAPE_HYPEN);
				}
				...
				if ("long".equals(type)) {
					...
					pd.setName(propName);
					pd.setType(PROPERTY_TYPES.DOUBLE);
				}  else if ("string".equals(type)) {
					pd.setName(propName);
					pd.setType(PROPERTY_TYPES.STRING);
				} else if ("datetime".equals(type)) {
					pd.setName(propName);
					pd.setType(PROPERTY_TYPES.DATE_TIME);
				}
...
//		destination.setSerializerDeserializerClass("---");
	...
	private static String removeInvalidChars(String replaceString) {
		if (replaceString.contains(" ")) {
			replaceString = replaceString.replace(" ", UNDERSCORE);
		}
		if (replaceString.contains(".")) {
			replaceString = replaceString.replace(".", UNDERSCORE);
		}
