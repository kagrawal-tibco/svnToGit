 ...
 *
 */
public class StateMachineSelectToolHandler extends SelectToolHandler{
	...
	public static void chooseState(AbstractButton button, TSEGraph graph, SelectTool tool, boolean isEditorEnabled)
	{
		...
		if( EntityResourceConstants.PASTE_GRAPH.equals(command)){
			...
			button.setEnabled(isClipBoardContentsAvailable(((StateMachineSelectTool)tool).getStateMachineDiagramManager()));
		}
			...
			if (member instanceof TSENode)
			{
				...
				if(type == EntityResourceConstants.CUT_GRAPH)
				{
					if(node.getUserObject() instanceof StateStart){
						...
						int size = StateMachineUtils.getStateEndSize(((StateComposite)node.getOwnerGraph().getUserObject()).getStateEntities());
						if(size == 1){
