...
public class TableAutoResizeLayout extends TableLayout implements ControlListener {
	 ...
	 * @param table
	 */
	public TableAutoResizeLayout(Table table) {
		this.table = table;
		table.addControlListener(this);
	}
	public TableAutoResizeLayout(Table table,Boolean autosizing) {
		this.table = table;
	 ...
	 * (0) //Resize based on content
	 * (1)//Get 50% of space
	 * (100)//Always 100 pixels
	 */
	...
	private void autoSizeColumns() {
		...
		for (int i = 0; i < size; i++) {
			...
			if (col instanceof ColumnPixelData) {
				...
				if (fixPixels == 0) {
					Point measurement = gc.textExtent(table.getColumn(i).getText());
