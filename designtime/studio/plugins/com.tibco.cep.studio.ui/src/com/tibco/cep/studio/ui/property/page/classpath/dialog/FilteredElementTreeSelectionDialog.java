...
public class FilteredElementTreeSelectionDialog extends ElementTreeSelectionDialog {
	...
	private static class MultiplePatternFilter extends PatternFilter {
		...
		private StringPatternMatcher[] fMatchers;
		...
		public void setPattern(String patternString) {
			...
			if (patternString != null && patternString.length() > 0) {
				ArrayList<StringPatternMatcher> res= new ArrayList<StringPatternMatcher>();
				StringTokenizer tok= new StringTokenizer(patternString, ",;"); //$NON-NLS-1$
				int tokenCount= tok.countTokens();
				for (int i= 0; i < tokenCount; i++) {
					String token= tok.nextToken();
					if (token.length() > 0) {
						res.add(new StringPatternMatcher(token + '*', true, false));
					}
				}
				if (!res.isEmpty()) {
					fMatchers= res.toArray(new StringPatternMatcher[res.size()]);
				}
			}
		}
	...
	protected TreeViewer doCreateTreeViewer(Composite parent, int style) {
		...
		ClasspathVariableUiUtils.setAccessibilityText(viewer.getControl(), removeMnemonicIndicator(getMessage()));
		return viewer;
	}
