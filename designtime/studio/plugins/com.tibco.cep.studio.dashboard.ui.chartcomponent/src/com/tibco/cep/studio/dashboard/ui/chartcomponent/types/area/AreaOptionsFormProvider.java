...
public class AreaOptionsFormProvider extends BaseChartGraphOptionsFormProvider {
	
	private PropertyControl plotShapeControl;
	private PropertyControl plotShapeDimensionControl;

	public AreaOptionsFormProvider(){
		super();
	}
	...
	protected BaseForm getTypeSpecificForm(FormToolkit formToolkit, Section section) {
		...
		plotShapeControl = form.addProperty("Plot Shape", ViewsConfigReader.getInstance().getProperty(BEViewsElementNames.AREA_CHART_VISUALIZATION, "PlotShape"));
		plotShapeDimensionControl = form.addProperty("Plot Shape Dimension", ViewsConfigReader.getInstance().getProperty(BEViewsElementNames.AREA_CHART_VISUALIZATION, "PlotShapeDimension"));
		form.addProperty("Line Thickness", ViewsConfigReader.getInstance().getProperty(BEViewsElementNames.AREA_CHART_VISUALIZATION, "LineThickness"));
		form.addProperty("Fill Opacity", ViewsConfigReader.getInstance().getProperty(BEViewsElementNames.AREA_CHART_VISUALIZATION, "FillingOpacity"));
		return form;
	}
	
	@Override
	public void setChartSubTypes(ChartSubType[] chartSubTypes) {
		if (Arrays.asList(chartSubTypes).contains(ChartTypeRegistry.getInstance().get("Area").getSubType("NoPoints")) == true){
