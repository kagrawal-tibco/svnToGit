 ...
 *
 */
public class PayloadDialog extends Dialog implements SelectionListener, ModifyListener {

	protected String title; 
	private Text payloadFilePathText;
	private Text payloadStringText;
	private Button browseButton;
	private String valueToBePassed;
	 ...
	 * @param column
	 */
	public PayloadDialog(Shell parentShell, 
			                  String dialogTitle, String initValue
			             ) {
		super(parentShell);
...
//		gd.heightHint = 100;
		...
		payloadStringText.setText( valueToBePassed == null ? "" : valueToBePassed);
	...
	protected void okPressed() {
		try {
			String text = payloadStringText.getText().trim();
		...
		} catch (SAXException | JsonParseException | JsonMappingException e) {
			MessageDialog.openError(getShell(), "Payload Error", "Invalid payload entry");
			return;
		} catch (IOException e) {
			e.printStackTrace();
		}
		copyValue(payloadStringText.getText());
		//	table.setValueAt(payloadStringText.getText(), row, col);
		super.okPressed();
	}
	
	private void parseJSON(String text) throws JsonParseException, JsonMappingException, IOException {
		ObjectMapper mapper = new ObjectMapper();
		mapper.readValue(text, JsonNode.class);
	}

	private void copyValue (String text){
		valueToBePassed = text;
	}
