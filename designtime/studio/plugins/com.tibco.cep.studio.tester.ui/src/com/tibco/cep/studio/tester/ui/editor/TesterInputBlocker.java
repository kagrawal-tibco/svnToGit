...
class TesterInputBlocker extends Dialog {
	static private TesterInputBlocker instance = null;
	static private int blockCount = 0;
	private Shell shell;

	private TesterInputBlocker(Shell parent) {
	...
	private Object open() {
		...
		shell.addFocusListener(new FocusAdapter() {
			public void focusGained(FocusEvent e) {
				...
				TesterSWTEnvironment.getInstance(shell.getDisplay())
	...
	static void block() {
		...
		if (blockCount == 1) {
			...
			Shell shell = TesterSWTEnvironment.getInstance(display).getShell();
			...
			if (shell != null) {
				instance = new TesterInputBlocker(shell);
				instance.open();
			}
		}
	}
