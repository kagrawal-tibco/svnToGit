 ...
 *
 */
public class ClearWorkingMemoryDebugEventSetListener implements
			...
			if (event.getKind() == DebugEvent.MODEL_SPECIFIC) {
				if(event.getDetail() == IRuleRunTarget.DEBUG_TASK_REPONSE) {
					...
					try {
						...
						if(data instanceof ClearWorkingMemoryContentsInputTask) {
							...
							WMContentsInputTask inputTask = new WMContentsInputTask(runTarget, 
																					((ClearWorkingMemoryContentsInputTask)data).getRuleSessionName(), 
																					numberOfObjects, 
																					true);
