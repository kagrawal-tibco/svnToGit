...
public class SBConfigModelParser extends AbstractSharedResourceModelParser {

	@Override
	public void loadModel(Document doc, SharedResModelMgr modelmgr) {
        Element root = doc.getDocumentElement();
        modelmgr.setRootAttributes(root.getAttributes());
        SBConfigModel model = (SBConfigModel) modelmgr.getModel();
...
//        	NamedNodeMap fileNodeAttr = fileNode.getAttributes();
        	if (isValidFileNode(fileNodeName) && fileNodeName.equalsIgnoreCase("sharedsb")) {
	 ...
	 */
	private static void processDescriptionNode(Node node, SBConfigModel model) {
	...
	public void saveModelParts(Document doc, SharedResModelMgr modelmgr) {
		Element root = doc.getDocumentElement();
		SBConfigModel model = (SBConfigModel) modelmgr.getModel();
		Element configNode = doc.createElement("sharedsb");
