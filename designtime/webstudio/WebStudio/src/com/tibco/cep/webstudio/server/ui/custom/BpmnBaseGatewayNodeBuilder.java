 ...
 * 
 */
public class BpmnBaseGatewayNodeBuilder extends TSPNodeBuilder {

	/*
	 * (non-Javadoc)
	 * 
	 * @see com.tomsawyer.graphicaldrawing.ui.TSPNodeBuilder#createNodeUI()
	 */
	@Override
	protected TSNodeUI createNodeUI() {
		// TODO Auto-generated method stub
		return this.getNodeUI();
	}
	...
	public void defineAttributeGroups(TSAttributeGroupManager groupManager) {
		groupManager.setGroupNames(Arrays.asList(GROUP_TEXT,
				GROUP_BACKGROUND_AND_BORDER, GROUP_SIZE_AND_POSITION));

		/*
		 * groupManager.setGroupAttributes(GROUP_TEXT, Arrays.asList( new
		 * TSTemplateAttribute( TSDrawingObjectPropertyHelper.PROPERTY_TOOLTIP,
		 * true), new TSTemplateAttribute(
		 * TSDrawingObjectPropertyHelper.PROPERTY_URL, true)));
		 * 
		 * 
		 * groupManager.setGroupAttributes(GROUP_BACKGROUND_AND_BORDER,
		 * Arrays.asList( new TSTemplateAttribute(
		 * BpmnNodeConstants.GRADIENT_COLOR_ENABLED), new TSTemplateAttribute(
		 * BpmnNodeConstants.BACKGROUND_COLOR), new TSTemplateAttribute(
		 * BpmnNodeConstants.GRADIENT_START_COLOR), new TSTemplateAttribute(
		 * BpmnNodeConstants.GRADIENT_FINISH_COLOR), new TSTemplateAttribute(
		 * BpmnNodeConstants.GRADIENT_DIRECTION), new TSTemplateAttribute(
		 * BpmnNodeConstants.BORDER_STYLE), new TSTemplateAttribute(
		 * BpmnNodeConstants.BORDER_COLOR), new TSTemplateAttribute(
		 * BpmnNodeConstants.BORDER_WIDTH)));
		 */

		groupManager.setGroupAttributes(GROUP_TEXT, getTextGroupAttributes());
	...
	protected List<TSTemplateAttribute> getTextGroupAttributes() {
		return Arrays.asList(new TSTemplateAttribute(
				TSDrawingObjectPropertyHelper.PROPERTY_TOOLTIP, true),
	...
	protected List<TSTemplateAttribute> getBackgroundAndBorderGroupAttributes() {
		return Arrays
				.asList(new TSTemplateAttribute(
						...
						new TSTemplateAttribute(BpmnNodeConstants.BORDER_STYLE),
						new TSTemplateAttribute(BpmnNodeConstants.BORDER_COLOR),
						new TSTemplateAttribute(BpmnNodeConstants.BORDER_WIDTH));
	}

	@Override
	public void defineAttributeDependencies(
			TSAttributeDependencyManager dependencyManager) {
		defineBackgroundAttributeDependencies(dependencyManager);
	}

	protected static void defineBackgroundAttributeDependencies(
			TSAttributeDependencyManager dependencyManager) {
		// gradient related

		TSTemplateAttribute gradientEnabledAttribute = new TSTemplateAttribute(
				BpmnNodeConstants.GRADIENT_COLOR_ENABLED);

		dependencyManager.setAttributeDependency(new TSTemplateAttribute(
				BpmnNodeConstants.BACKGROUND_COLOR), gradientEnabledAttribute,
				Boolean.FALSE);

		dependencyManager.setAttributeDependency(new TSTemplateAttribute(
				BpmnNodeConstants.GRADIENT_START_COLOR),
				gradientEnabledAttribute, Boolean.TRUE);
