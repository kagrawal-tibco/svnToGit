...
public class DecisionTable {
    ...
    public void addRuleTupleInfo(RuleTupleInfo ri) {
    	idToRuleTuples.put(ri.getId(), ri);
    }
    
    public void removeRuleTupleInfo(RuleTupleInfo ri) {
    	idToRuleTuples.remove(ri.getId());
    }
    ...
    public void addCellAtDTCreation(Cell c) {
    	if (c != null) {
            if (c.getAlias() != null && c.getCellInfo().getAlias() != null) {
                LinkedList<Cell> cells = conditionMatrix.get(c.getCellInfo().getAlias());
                if (cells == null) {
                    cells = new LinkedList<Cell>();
                    conditionMatrix.put(c.getCellInfo().getAlias(), cells);
...
//    	constraintsTableListener.cellUpdated(event);
    ...
    private void toggleActionCell(String alias,
		...
		String ruleId = DecisionConstraintUtils.getContainingRuleId(trvId);
		List<Cell> cells = Collections.emptyList();
		for (List<Cell> aliasCells : conditionMatrix.values()) {
			if (!aliasCells.isEmpty()) {
				for (Cell cell : aliasCells) {
					String condRuleId = DecisionConstraintUtils.getContainingRuleId(cell.getId());
    ...
    public void updateCell(String alias, 
    	...
    	String cellId = tableRuleVariable.getId();
    	String expression = tableRuleVariable.getExpression();
//    	DecisionTableUIPlugin.debug(CLASS, "Caling Update Cell for Cell Id {0} and expression {1}", cellId, expression);
        if (alias != null && cellId != null) {
        	...
        	case CUSTOM_CONDITION :
                ...
                if (conditionCells == null) {
                	...
                	for (Cell c : conditionCells) {
                		if (c.getId().intern() == cellId.intern()) {
                			cellsToRemove.add(c);
                		}
                	}
        	...
        	case CUSTOM_ACTION :
        		...
        		String ruleId = DecisionConstraintUtils.getContainingRuleId(cellId);
        		...
        		if (ruleId != null) {
        			...
        			if (ruleTupleInfo != null) {
	    				...
	    				for (Cell c : actionCells) {
	    					if (c.getId().intern().equals(cellId.intern())) {
	    						cellsToRemove.add(c);
	    					}
	    				}
...
//    	fireCellUpdated(oldCells, updatedCells);
    ...
    public void removeCell(Cell cell) {
    	 if (cell != null) {
				...
				switch (cellType) {
				case 0:
					List<Cell> cells = conditionMatrix.get(cell.getCellInfo().getAlias());
					if (cells != null) {
						cells.remove(cell);
				...
				case 1:
					...
					// ruletupleinfo
					String ruleId = DecisionConstraintUtils.getContainingRuleId(cell.getId());
					// Search for this in the hashmap
					if (ruleId != null) {
						RuleTupleInfo ruleTupleInfo = idToRuleTuples.get(ruleId);
						if (ruleTupleInfo != null) {
