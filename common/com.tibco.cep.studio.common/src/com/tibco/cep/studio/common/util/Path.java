...
public class Path {
	...
	public static final String JAVA_SRC_PATH = "/JavaSrc";
	...
	public Path setDevice(String value) {
		if (value != null) {
			assert (value.indexOf(Path.DEVICE_SEPARATOR) == (value.length() - 1));
	...
	public Path uptoSegment(int count) {
		...
		if (count >= segments.length)
			return this;
		assert (count > 0);
		// "Invalid parameter to Path.uptoSegment"); //$NON-NLS-1$
