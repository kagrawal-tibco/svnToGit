...
public class EventParserTester {
	...
	public static void main(String[] args) {
		...
		try {
			 ...
			 */
			EventLexer lexer = new EventLexer(new ANTLRFileStream(args[0], ModelUtils.DEFAULT_ENCODING));
			CommonTokenStream tokens = new CommonTokenStream(lexer);
			EventParser parser = new EventParser(tokens);
			...
			if (startRule != null) {
				...
				((RulesASTNode)tree).setData("tokens", parser.getTokenStream());
				...
				new EventParserTester().convertEventASTToString((RulesASTNode)tree);
	...
	public String convertEventASTToString(RulesASTNode conceptNode) throws IOException {
		StringTemplateGroup stgroup = new StringTemplateGroup(new InputStreamReader(getClass().getResourceAsStream("/com/tibco/cep/studio/core/grammar/event/EventTemplate.stg")));

		StringTemplate eventTemplate = stgroup.getInstanceOf("Event_Definition");
		eventTemplate.reset();
		
		EventInfo eventInfo = getEventInfo(conceptNode);
    	eventTemplate.setAttribute("elementInfo", eventInfo);
    	System.out.println(eventTemplate.toString());
    	...
    	eventTemplate.write(new AutoIndentWriter(mapperStreamWriter));
		...
		return eventTemplate.toString();
	}

	private EventInfo getEventInfo(RulesASTNode eventNode) {
    	EventCreatorASTVisitor visitor = new EventCreatorASTVisitor("Test", "Test/eventName.event");
    	eventNode.accept(visitor);
    	Event createdEvent = visitor.getEvent();
    	EventInfo info = new EventInfo(createdEvent);
    	return info;
	}
