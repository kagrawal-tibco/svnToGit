/**
 * Created by Priyanka on 08/06/15
 */
beConfigModule.controller("processingUnitAgentCtrl",['$scope','$location','$rootScope','$q','$http','$timeout','teaLocation', 'teaObjectService','ReferenceObjectConfigService','teaEventNotifications','StorageService','teaEventNotifications','utilService',
	...
	$rootScope.loadingComplete.then(function( apps ){
			...
			$scope.processingUnitAgent = ReferenceObjectConfigService.getSelectedProcessingUnitAgent();
			$scope.search={};
			$scope.check={};
			...
			$scope.processingUnitAgentInstances;
			$scope.instanceAgents = [];
			$scope.selectedInstancesAgents = [];
			$scope.isOnAgentView= true;
			...
			ReferenceObjectConfigService.getBreadCrumbObject().push($scope.processingUnitAgent);
			if(Storage)
			...
			$scope.setViewType = function(viewType){
		    	...
		    	if(viewType == "operation"){
		        	$scope.runningInstances=[];
		        	$scope.tempInstances = angular.copy($scope.processingUnitAgentInstances);
		    ...
		    $scope.filteredList = angular.copy($scope.processingUnitAgentInstances);
		    ...
		    $scope.doSubscribe = function() {
		        if (subscribe && $scope.processingUnitAgentInstances!=undefined) {
		            subscribe = false;
		            angular.forEach($scope.processingUnitAgentInstances, function(instance) {
			...
			$scope.filterList1 = function(){
					$scope.filteredList = angular.copy($scope.processingUnitAgentInstances);			
					for (var i = $scope.processingUnitAgentInstances.length - 1; i >= 0; i--) {
						if(!((isContainsHost($scope.processingUnitAgentInstances[i]))&& (isContainsPU($scope.processingUnitAgentInstances[i])) && (isContainsName($scope.processingUnitAgentInstances[i])))) {
			...
			$scope.operation = function(){
				 ...
				 if($scope.isMultipleInstanceDelete){
					 ...
					 teaObjectService.invoke({
						 	'agentID'  : $scope.processingUnitAgent.agentId,
				            'agentType': $scope.processingUnitAgent.type.agentType,
				       		'objectType': $scope.processingUnitAgent.type.name,           		
				       		'objectKey': $scope.processingUnitAgent.key,
				       		'operation' : 'groupDelete',
				       		'methodType': 'UPDATE',
				       		'params': {
								instances : selectedInstancesList,
								version: $scope.instanceData.host.application.version
							}
						 }).then(function (resp) {	
							$('#loaderdiv').hide();
			...
			$scope.startInstance =function(index){
				var instanceToStart = $scope.processingUnitAgentInstances[index];
				$('#loaderdiv').show(); 
				 teaObjectService.invoke({
			...
			$scope.stopInstance =function(index){
				var instanceToStop = $scope.processingUnitAgentInstances[index];
				$('#loaderdiv').show(); 
				 teaObjectService.invoke({
		    ...
		    function getAgentInstances(){
		    	teaObjectService.reference({
		            ...
		            'agentType': $scope.processingUnitAgent.type.agentType,
		       		'objectType': $scope.processingUnitAgent.type.name,           		
		       		'objectKey': $scope.processingUnitAgent.key,
		       		'reference' : 'AgentServiceInstance'
				}).then(function (instances) {					
					$scope.processingUnitAgentInstances = instances;
					getInstanceInfo();
					getInstanceMemoryUsage();
					getInstanceCPUUsage();
					$scope.doSubscribe();
					$('#loaderdiv').hide(); 
				}, function (failReason) {
			...
			function getInstanceInfo(){
				...
				teaObjectService.reference({
						...
						'reference' : 'ServiceInstances'
	            			...
	            			$scope.processingUnitAgentInstances = instances;
	            			var instancesList = $scope.processingUnitAgentInstances;
	            		...
	            		defer.reject('Error retrieving processing unit agent instances');
	            	});
			...
			$scope.getOperationInfo = function(method,group){
		    	...
		    	for(var i=0 ; i< $scope.parameters.length ; i++){
					...
					if(param.name == "Session"||'sessionName' === param.name){
						param.defaultvalue = $scope.processingUnitAgent.name;
					}
				}
		    	$scope.result={};
			...
			function getInstanceMemoryUsage(){			
				angular.forEach($scope.processingUnitAgentInstances, function(instance,index) {
					if(instance.status.state == "Running" ){
						teaObjectService.invoke({
							'agentID'  : instance.agentId,
							'agentType': instance.type.agentType,
							...
							$('#loaderdiv').hide();
							$scope.processingUnitAgentInstances[index].memory = info[0].percentUsed + "%";
							$scope.filteredList = angular.copy($scope.processingUnitAgentInstances);
						}, function (failReason) {
							$('#loaderdiv').hide(); 
						});	 
					}else
						$scope.processingUnitAgentInstances[index].memory = "";
				});	
			}
				...
				angular.forEach($scope.processingUnitAgentInstances, function(instance,index) {
					if(instance.status.state == "Running" ){
						teaObjectService.invoke({
							...
							$('#loaderdiv').hide();
							$scope.processingUnitAgentInstances[index].cpu = info.cpuUsageInPercent + "%";
							$scope.filteredList = angular.copy($scope.processingUnitAgentInstances);
						}, function (failReason) {
							$('#loaderdiv').hide(); 
						});	 
					}else
						$scope.processingUnitAgentInstances[index].cpu = "";
				});	
			}
			...
			$scope.showIndividualOperations = function(){
				...
				for (var i = 0; i < $scope.filteredList.length; i++) {
		            ...
		            if ($scope.filteredList[i].isChecked && ($scope.filteredList[i].config.deploymentStatus == 'Deployed' || $scope.filteredList[i].config.deploymentStatus == 'Needs Re-deployment')) {
