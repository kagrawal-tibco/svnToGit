beConfigModule.controller("instanceController",['$scope','$location','$window','$http','fileUpload','$q','$rootScope', '$timeout','teaLocation', 'teaObjectService','ReferenceObjectConfigService','teaEventNotifications','StorageService','utilService','MetricViewService',
	...
	$rootScope.loadingComplete.then(function( apps ){
			...
			$scope.filterFunction = function(gv){
				if(($scope.search.name == undefined || $scope.search.name == null || $scope.search.name == "" || (angular.uppercase(gv.name).indexOf(angular.uppercase($scope.search.name)) > -1)))
				 	return true;
				else
					return false;
			}
			...
			function getInstanceInfo(){
				...
				teaObjectService.invoke({
					...
					/*if($scope.instanceInfo.deploymentPath.indexOf("\\") > -1){
						$scope.instanceInfo.deploymentPath = $scope.instanceInfo.deploymentPath + "\\" + ReferenceObjectConfigService.getSelectedApp().name;
					}else if($scope.instanceInfo.deploymentPath.indexOf("/") > -1)
						$scope.instanceInfo.deploymentPath = $scope.instanceInfo.deploymentPath + "/" + ReferenceObjectConfigService.getSelectedApp().name;*/
			...
			/*function getLoggersInfo(){
				teaObjectService.invoke({
					'agentID'  : $scope.instance.agentId,
		            'agentType': $scope.instance.type.agentType,
		       		'objectType': $scope.instance.type.name,           		
		       		'objectKey': $scope.instance.key,
		       		'operation' : 'getLoggerLogLevels',
		       		'methodType':'READ'
				}).then(function (loggers) {
					$scope.loggersInfo=loggers;	
					$('#loaderdiv').hide();
				}, function (failReason) {
					$('#loaderdiv').hide();
					console.log("failed");
			...
			/*$scope.isGlobalVariableChange = function(){
				$scope.isGlobalVariableChanged = false;
				for(var i =0 ; i < $scope.globalVariables.nameValuePairs.nameValuePair.length ; i++){
					var newValue = $scope.globalVariables.nameValuePairs.nameValuePair[i].value;
					var oldValue = $scope.tempGlobalVariable.nameValuePairs.nameValuePair[i].value;
					if((!(newValue == "" && oldValue== null ) && (newValue != oldValue)) || ($scope.globalVariables.nameValuePairs.nameValuePair[i].isDeleted !=$scope.tempGlobalVariable.nameValuePairs.nameValuePair[i].isDeleted )){
						$scope.isGlobalVariableChanged = true;
						break;
					}
				}
			}*/
			...
			/*function getInstanceAudit(){
				teaObjectService.invoke({
					'agentID'  : $scope.instance.agentId,
					'agentType': $scope.instance.type.agentType,
					'objectType': $scope.instance.type.name,           		
					'objectKey': $scope.instance.key,
					'operation' : 'getInstanceAudit',
					'methodType':'READ'
				}).then(function (info) {			
					$scope.auditInfo=info;
					$scope.tableParams = new ngTableParams({
				          page: 1,     
				          count: 20
				      },{
				          total: $scope.auditInfo.auditRecord.length,
				          getData: function($defer, params) {
				              var orderedData = $scope.auditInfo.auditRecord;
				              params.total(orderedData.length);
				              $defer.resolve(orderedData.slice((params.page() - 1) * params.count(), params.page() * params.count()));
				          }
				    });
					angular.forEach($scope.auditInfo.auditRecord, function(rec,index) {
						var date = new Date(rec.performedOn);
						var curr_date = date.getDate();
						var curr_month = date.getMonth() + 1;
						var curr_year = date.getFullYear();
						$scope.auditInfo.auditRecord[index].performedOn =curr_date + "-" + curr_month + "-" + curr_year;
					});
					$('#loaderdiv').hide();
				}, function (failReason) {
					console.log("failed");
			...
			$scope.showLogDetails = function(lines,isASLog){
				teaObjectService.invoke({
					...
					$scope.isASLog = isASLog;
					$('#loaderdiv').hide(); 
					$scope.logDetails = data + "\n\n\n\n\n\n\n\n\n\n\n\n"; //empty lines added so that last lines should be readable BE-24366
