beConfigModule.controller("applicationManagmentCtrl",['$scope','$sce','$q','$location','$rootScope', '$timeout', 'fileUpload','teaLocation','teaEventNotifications' ,'teaObjectService', 'teaScopeDecorator','$http','StorageService','ReferenceObjectConfigService','utilService',
                                       function ($scope,$sce,$q,$location, $rootScope,$timeout,fileUpload, teaLocation, teaEventNotifications,teaObjectService, teaScopeDecorator ,$http,StorageService,ReferenceObjectConfigService,utilService){
	$('#loaderdiv').show();
	...
	$scope.isFileExtensionST = false;
	$scope.isFileExtensionCDD = false;
	$scope.isFileExtensionEAR = false;
	$scope.isFileExtensionZIP = false;
	$scope.isFileExtensionTRA = false;
    ...
    $scope.doSubscribe = function() {
    	if (subscribe) {
            subscribe = false;
            angular.forEach($scope.appObject, function(app) {
                teaEventNotifications.addObjectIdToSubscription(app.agentId,  app.type.agentType,  app.type.name, "", [Event.STATUS_CHANGE]);
	...
	$scope.validateFileExtension = function(filename, fileExtension){
		var filesExtension = filename.split(".")[filename.split(".").length -1];
		
		if(fileExtension == "cdd"){
			if(filesExtension != fileExtension)
				$scope.isFileExtensionCDD = true;
			else
				$scope.isFileExtensionCDD = false;
		}
		else if(fileExtension == "ear"){
			if(fileExtension != filesExtension)
				$scope.isFileExtensionEAR = true;
			else
				$scope.isFileExtensionEAR = false;
		}
		else if(fileExtension.toLowerCase() == "tra"){
			if(fileExtension.toLowerCase() != filesExtension)
				$scope.isFileExtensionTRA = true;
			else
				$scope.isFileExtensionTRA = false;
		}
		else{
			if(fileExtension != filesExtension)
				$scope.isFileExtensionZIP = true;
			else
				$scope.isFileExtensionZIP = false;
		}
	}
	$scope.readFileContent = function(filename, $fileContent, fileExtension){
		var filesExtension = filename.split(".")[filename.split(".").length -1];
		
		if(filesExtension != fileExtension)
			$scope.isFileExtensionST = true;
		else
			$scope.isFileExtensionST = false;
		if(!$scope.isFileExtensionST){
	...
	$scope.removeTraAndLogUpdate = function(index){
		...
		$scope.traAndLogUpdates.splice(index, 1);
		for(var i = 0; i < $scope.traAndLogUpdates.length; i++){
			$scope.checkIfTRAConfigAlreadyExists(i, $scope.traAndLogUpdates[i].hostId);
		}
	}
	...
	$scope.createApplication = function(applicationName,CDDfilesCreateApp,earfilesCreateApp) {
		$('#loaderdiv').show(); 
		var fd = new FormData();       
