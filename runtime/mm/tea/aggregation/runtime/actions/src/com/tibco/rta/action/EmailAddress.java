...
public class EmailAddress implements Serializable
{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -8952048416089462679L;
...
//	private static final String phrase = word + "+"; //one or more words. [causing an infinite computation in long email address]
	private static final String phrase = word + "\\d{0,251}"; //(between 0 to 250 occurrences )minimum one word and maximum up to 250 words.
	...
	*/
	...
	public static boolean isValidEmailAddress(String emailAddresses)
	{
		StringTokenizer emailAddrTokenizer = new java.util.StringTokenizer(emailAddresses,";,");
		if (emailAddresses == null || emailAddresses.length() == 0)
			return false;
		while (emailAddrTokenizer.hasMoreTokens())
		{
		    String emailAddress = emailAddrTokenizer.nextToken();
		    if (!VALID_PATTERN.matcher(emailAddress).matches())
		   	 return false;
		}
		return true;
	}
	...
	public static void main(String[] args) {
		...
		if (args.length > 0) {
			for (int i = 0; i < args.length; i++) {
				emailAdrress = emailAdrress + args[i];
			}
		} else {
			emailAdrress = "\"John Smith\"<jsmith@special.com>";
		}
		if (isValidText(emailAdrress)) {
			System.out.println(emailAdrress + " is a Valid email address.");
		} else {
			System.out.println(emailAdrress + " is an INVALID email address!");
		}
	}
}