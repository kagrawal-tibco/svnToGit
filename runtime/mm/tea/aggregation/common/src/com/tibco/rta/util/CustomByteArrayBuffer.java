 ...
 * Date: 21/2/13
 * Time: 7:12 PM
 * A custom byte buffer (courtesy Apache HTTP core) along with couple of
 * convenience methods.
 */
public class CustomByteArrayBuffer {
    ...
    /**
     * Creates an instance of {@link CustomByteArrayBuffer} with the given initial
     * capacity.
     *
     * @param capacity the capacity
     */
    public CustomByteArrayBuffer(int capacity) {
        super();
        if (capacity < 0) {
...
//        byte newbuffer[] = new byte[Math.max(this.buffer.length << 1, newlen)];
     ...
     * @return byte array
     */
    public byte[] toByteArray(int startPos, int length) {
        byte[] b = new byte[length - startPos];
        if (length > 0) {
            System.arraycopy(this.buffer, startPos, b, 0, length - startPos);
        }
        return b;
    }
