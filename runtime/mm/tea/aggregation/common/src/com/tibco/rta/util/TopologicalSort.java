...
public final class TopologicalSort {
     ...
     */
    public static <T> List<T> sort(DiGraph<T> g) {
        /* Construct the reverse graph from the input graph. */
        DiGraph<T> gRev = reverseGraph(g);
     ...
     */
    private static <T> void explore(T node, DiGraph<T> g,
        ...
        if (visited.contains(node)) {
            ...
            throw new IllegalArgumentException("Measurement dependencies are cyclic");
        }
     ...
     */
    private static <T> DiGraph<T> reverseGraph(DiGraph<T> g) {
