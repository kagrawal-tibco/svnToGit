...
/**
 * This metric will return the percentage ok value as  follows:
 * -----AGENT DIM LEVEL    : return -1 : not applicable at this level
 * -----INSTANCE DIM LEVEL : return -1 : not applicable at this level 
 * -----APP DIM LEVEL      : This is a generic method to evaluate %ok value for both agent and instance
 ...
 *
 */
public class PercentOkHealthMetric extends SingleValueMetricFunction<Long> {
		...
		if(metricKey.getDimensionLevelName().equals(MetricAttribute.DIM_APP)) {
			...
			if(entityType.equals(MetricAttribute.PU_INSTANCE_HEALTH)){	
				...
				for(String instanceName : rawHealthContext.getTupleNames()) {
					...
					if(BEEntityHealthStatus.ok.getHealthStatus().equals(healthValue)) {
						num++;
					}
				}
				...
				for(String instanceName : rawHealthContext.getTupleNames()) {
					...
					for(Map.Entry<String, String> entry : agentHealthMap.entrySet() ){
						if(BEEntityHealthStatus.ok.getHealthStatus().equals(entry.getValue())) {
							num++;
						}
					}
				}
