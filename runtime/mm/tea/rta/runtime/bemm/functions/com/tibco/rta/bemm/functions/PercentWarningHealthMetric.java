...
/**
 * This metric will return the percentage warning value as  follows:
 * -----AGENT DIM LEVEL    : return -1 : not applicable at this level
 * -----INSTANCE DIM LEVEL : return -1 : not applicable at this level 
 * -----APP DIM LEVEL      : This is a generic method to evaluate %warning value for both agent and instance
 ...
 *
 */
public class PercentWarningHealthMetric extends SingleValueMetricFunction<Long> {
	...
	public Long compute(MetricNode metricNode, SingleValueMetric<Long> metric, RtaNodeContext context) {
		...
		if(metricKey.getDimensionLevelName().equals(MetricAttribute.DIM_APP)) {
			...
			if(entityType.equals(MetricAttribute.PU_INSTANCE_HEALTH)){	
				...
				for(String puName : rawHealthContext.getTupleNames()) {
					String healthValue = (String) rawHealthContext.getTupleValue(puName);
					if(BEEntityHealthStatus.warning.getHealthStatus().equals(healthValue)) {
						num++;
					}
				}
				...
				for(String instanceName : rawHealthContext.getTupleNames()) {
					...
					for(Map.Entry<String, String> entry : agentHealthMap.entrySet() ){
						if(BEEntityHealthStatus.warning.getHealthStatus().equals(entry.getValue())) {
							num++;
						}
					}
				}
				SingleValueMetric<Object> instanceCountMetric = (SingleValueMetric<Object>) metricNode.getMetric(MetricAttribute.AGENT_COUNT);
				Long total=getAttributeValue(instanceCountMetric.getValue());
				return (total==0)?0:((num*100l)/total);
			}
			else
