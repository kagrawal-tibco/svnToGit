...
*/
public class MultiPatternIntegNegTest extends AbstractMultiPatternIntegTest {
    public static void main(String[] args) throws LifecycleException, RoutingException {
        ...
        MultiPatternIntegNegTest test = new MultiPatternIntegNegTest();
    ...
    public DriverTracker setUpPattern(Admin admin) throws LifecycleException, RoutingException {
        ...
        session.definePatternSubscription()
                ...
                .alsoListenTo(cancelRequestED.getResourceId())
                .as("cr").use("accountId");
    ...
    protected void verify(DriverTracker driverTracker, int TOTAL) {
        ...
        for (DriverTrace driverTrace : driverTraces) {
            ...
            if (response instanceof Complete) {
                ...
                DefaultId driverId = (DefaultId) driverTrace.getDriverId();
                int testNumber = Integer.parseInt(driverId.getParent().toString());

                Assert.assertTrue((testNumber % 2) == 0,
                        "The test number that succedded [" + testNumber +
                                "] is wrong. Only odd numbered tests should've failed.");
            }
        }

        Assert.assertEquals(successCount, TOTAL / 2, "Some Drivers did not complete successfully.");
    }
}