...
import com.tibco.cep.query.stream.impl.query.ContinuousQueryImpl_V2;
...
/*
 * Implementation copied from ReteQueryImpl.java
 * Author: Karthikeyan Subramanian / Date: Jan 27, 2010 / Time: 12:35:46 PM
 */

public class ReteQueryImpl_V2 extends ContinuousQueryImpl_V2<GenericReteEntityHandle>
     ...
     */
    public ReteQueryImpl_V2(String regionName, ResourceId resourceId, ReteEntitySource[] reteEntitySources,
     ...
     *                               "required" value.
     */
    protected ReteQueryImpl_V2(String regionName, ResourceId resourceId,
    ...
    @Override
    public Map<Class, ReteEntitySource[]> getReteEntityClassAndListeners() {
        return reteEntityClassAndListeners;
    }

    @Override
    public Map<String, ReteEntityFilter[]> getReteEntityResourceIdAndFilters() {
    ...
    /**
     * Invokes {@link com.tibco.cep.query.stream.query.continuous.AbstractContinuousQuery#init(java.util.Map)}
     *
     ...
     * @throws Exception
     */
    public final void enqueueInput(ReteEntityBatchHandle handle, Sender sender) throws Exception {
        switch (sender) {
            case CURRENT: {
    ...
    private void accumulateCurrent(GenericReteEntityHandle handle) throws InterruptedException {
        if (accumulatedItemsDuringSSA != null) {
            ...
            try {
                ...
                throw new InterruptedException(sanitizeMessage(ex));
            }
        }
    }
    ...
    public void performSyncWork() {
        try {
            // If the job is not running or not in the queue,
            // Request schedule.
            if(scheduler.estimateRunTime(jobId) <= 0) {
                scheduler.scheduleNow(jobId);
            }
        } catch(SchedulerException e) {
            logger.log(Level.ERROR, "Unable to request schedule for Job-" + jobId);
        }
    }
