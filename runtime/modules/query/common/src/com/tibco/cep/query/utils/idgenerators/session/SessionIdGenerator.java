package com.tibco.cep.query.utils.idgenerators.session;
...
public class SessionIdGenerator extends AbstractStringIdentifierGenerator implements Serializable {
    ...
    public long maxLength() {
        return com.tibco.cep.query.utils.idgenerators.session.SessionIdGenerator.RANDOM_LENGTH + com.tibco.cep.query.utils.idgenerators.session.SessionIdGenerator.TIME_LENGTH
            + AbstractStringIdentifierGenerator.MAX_INT_ALPHANUMERIC_VALUE_LENGTH;
    }

    public long minLength() {
        return com.tibco.cep.query.utils.idgenerators.session.SessionIdGenerator.RANDOM_LENGTH + com.tibco.cep.query.utils.idgenerators.session.SessionIdGenerator.TIME_LENGTH + 1;
    }
    ...
    public String nextStringIdentifier() {
        ...
        // Random value
        //--------------
        long currentRandom = com.tibco.cep.query.utils.idgenerators.session.SessionIdGenerator.randomizer.nextLong();
        if (currentRandom < 0) {
        ...
        currentRandom %= com.tibco.cep.query.utils.idgenerators.session.SessionIdGenerator.MAX_RANDOM_LEN;
        currentRandom += com.tibco.cep.query.utils.idgenerators.session.SessionIdGenerator.MAX_RANDOM_LEN;
        ...
        synchronized (this) {
            // Time
            //--------------
            currentTimeValue = (System.currentTimeMillis() / com.tibco.cep.query.utils.idgenerators.session.SessionIdGenerator.TIC_DIFFERENCE);

            // force value into 3 char range, and add to pad with zeros
            // this gives a length of 4, when converted to base 36, and
            // the first character (always 1 from the add) is dropped
            currentTimeValue %= com.tibco.cep.query.utils.idgenerators.session.SessionIdGenerator.MAX_TIME_SECTION_LEN;
            currentTimeValue += com.tibco.cep.query.utils.idgenerators.session.SessionIdGenerator.MAX_TIME_SECTION_LEN;
