...
import com.tibco.cep.query.stream.impl.rete.join.NotExistsPeteyImpl;
...
public class JoinedWithNotExistsStreamTest extends AbstractTest {
    ...
    public void test() {
        try {
            ...
            ExistsPeteyImpl petey = new NotExistsPeteyImpl(peteyInfo);

            aliases = new HashMap<String, Class<? extends Tuple>>();
            aliases.put("orderStream", OrderTuple.class);
            SimpleExpression simpleExpression = new SimpleExpression(aliases,
                    "orderStream.getColumn(0).customerId == \"Teriyaki\"");

            ComplexAndExpression andExpression = new ComplexAndExpression(new AbstractExpression[]{
                    simpleExpression, new ExistsSimpleExpression(petey)});
            ...
            order = new Order();
            order.setCustomerId("Aryabhatta");
            order.setOrderId("ARYABHATTA452235G");
            data = new OrderTuple(SimpleIdGenerator.generateNewId(), new Object[]{order});
            newOrders.add(data);
            allOrders.add(data);
            ...
            order = new Order();
            order.setCustomerId("Teriyaki");
            order.setOrderId("TERIYAKI333333T3F");
            data = new OrderTuple(SimpleIdGenerator.generateNewId(), new Object[]{order});
            newOrders.add(data);
            expectedOrders.add(data);
            allOrders.add(data);
            ...
            order = new Order();
            order.setCustomerId("Sellers");
            order.setOrderId("SELLERSWWW334334");
            data = new OrderTuple(SimpleIdGenerator.generateNewId(), new Object[]{order});
            newOrders.add(data);
            allOrders.add(data);
            ...
            order = new Order();
            order.setCustomerId("Simpson");
            order.setOrderId("SIMPSONEEWERX4255");
            data = new OrderTuple(SimpleIdGenerator.generateNewId(), new Object[]{order});
            newOrders.add(data);
            allOrders.add(data);
