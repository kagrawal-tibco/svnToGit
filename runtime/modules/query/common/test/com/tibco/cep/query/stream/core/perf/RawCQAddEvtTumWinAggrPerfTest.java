...
public class RawCQAddEvtTumWinAggrPerfTest extends AbstractTest {
    ...
    protected void runTest() throws Exception {
        ...
        WindowBuilder builder = new WindowBuilder() {
            public Window buildAndInit(ResourceId parentResourceId, String id, GroupKey groupKey,
                ...
                TumblingWindow tumblingWindow = new TumblingWindow(new ResourceId(parentResourceId,
                        "Tumbling:" + id), tupleInfo, groupKey, windowOwner,
                        (TumblingWindowInfo) windowInfo);

                String[] names = new String[]{"count", "maxAge"};
                Class[] types = new Class[]{Integer.class, Integer.class};

                ISubStream iSubStream =
                        new ISubStream(tumblingWindow, new ResourceId("IStream:" + id));

                AggregatedStream aggregatedStream = new AggregatedStream(iSubStream,
                        new ResourceId(parentResourceId, "Aggregate:" + id), aggregateInfo, true);

                tumblingWindow.init(aggregateTransformer, aggregatedStream, null);

                return tumblingWindow;
            }
        };

        TumblingWindowInfo windowInfo = new TumblingWindowInfo(Integer.MAX_VALUE);
        ...
        CQ query = new CQ(master.getAgentService().getName(),
                new ResourceId("TumblingWindowAggregateQry"), new Source[]{source}, sink);
    ...
    private long pollAndCollect(int totalEventTypes, int lastEvent, StreamedSink streamedSink,
        ...
        int countExpected = lastEvent / totalEventTypes;

        for (String name : nameToCount.keySet()) {
            Integer i = nameToCount.get(name);

            Assert.assertEquals(i.intValue(), countExpected, "Incorrect window count for: " + name);
        }
    ...
    protected void doAdds(final CQ query, final int startId, final int lastId) throws Exception {
        Thread t = new Thread() {
            ...
            public void run() {
                ...
                final int batch = 500;
                WrappedCustomCollection<Tuple> queue =
                        new WrappedCustomCollection(new ArrayList<Tuple>(batch));
                int c = 0;
                for (int i = startId; i <= lastId; i++) {
                    ...
                    if (c == batch - 1) {
                        try {
                            query.enqueueInput(queue);
                        }
                        catch (Exception e) {
                            e.printStackTrace(System.err);
                        }

                        queue = new WrappedCustomCollection(new ArrayList<Tuple>(batch));
                    }
    ...
    public static class CQ extends ContinuousQueryImpl {
        ...
        protected void handleQueuedInput(Object object) throws Exception {
            sources[0].sendNewTuples(context, (WrappedCustomCollection<Tuple>) object);
        }
    }
}