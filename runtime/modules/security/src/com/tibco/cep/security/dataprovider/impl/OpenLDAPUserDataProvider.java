...
public class OpenLDAPUserDataProvider extends LDAPUserDataProvider {
	...
	public Iterator<Role> getUserRoles(final User user) throws DataProviderException {	
		...
		try {
			...
			//Initialise Env
			Hashtable<Object, Object> dynEnv = this.initializeDynEnv("simple", userDN, user.getPassword());

			//Base DN
			String baseDN = this.getProperty(DataProviderConstants.PROP_LDAP_BASEDN, null);
			LOGGER.log(Level.DEBUG, "Base DN for search is %s", baseDN);

			//Build Search Filter
			LOGGER.log(Level.DEBUG, "User DN for fetching roles is %s", userDN);
			String roleAttr = this.getProperty(DataProviderConstants.PROP_LDAP_ROLEATTR, "member");
	        ...
	        boolean useRoleDN = Boolean.parseBoolean(this.getProperty(DataProviderConstants.PROP_USE_ROLE_DN, "true"));
			String roleNameAttr = useRoleDN ? this.getProperty(DataProviderConstants.PROP_LDAP_DN_ATTR, "entryDN")
