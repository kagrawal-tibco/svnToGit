...
public class TestNotLikeCondition {
	...
	public void test() throws Exception {
		evaluator = new ASTWalker("$testName not like \"nest.*\"");
		Map<String, Object> bindValues = new HashMap<String, Object>();
		bindValues.put("testName", "tester2");
		Assert.assertEquals(evaluator.evaluate(concept, bindValues), true);
		evaluator = new ASTWalker("testStringField not like \"nest.*\"");
		Assert.assertEquals(evaluator.evaluate(concept), true);
		evaluator = new ASTWalker("@extId not like \"nest.*\"");
		Assert.assertEquals(evaluator.evaluate(concept), true);
	}
