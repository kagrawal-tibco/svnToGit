...
public class TestConcatCondition {
	...
	public void test() throws Exception {
		evaluator = new ASTWalker("$testName like \"test.*\" + \"1\"");
		Map<String, Object> bindValues = new HashMap<String, Object>();
		bindValues.put("testName", "tester1");
		Assert.assertEquals(evaluator.evaluate(concept, bindValues), true);
		evaluator = new ASTWalker("testStringField like \"test.*\" + \"1\"");
		Assert.assertEquals(evaluator.evaluate(concept), true);
		evaluator = new ASTWalker("testStringField = \"tester\" + \"1\"");
		Assert.assertEquals(evaluator.evaluate(concept), true);
		evaluator = new ASTWalker("testStringField = \"tes\" + \"ter\" + \"1\"");
		Assert.assertEquals(evaluator.evaluate(concept), true);
		evaluator = new ASTWalker("testStringField = \"tes\" + \"ter\" || " + "\"1\"");
		Assert.assertEquals(evaluator.evaluate(concept), true);
		evaluator = new ASTWalker("@extId like \"test.*\" + \"1\"");
