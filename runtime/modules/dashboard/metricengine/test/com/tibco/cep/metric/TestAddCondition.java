...
public class TestAddCondition {
	...
	public void test() throws Exception {
		evaluator = new ASTWalker("$testId = 0+1");
		...
		evaluator = new ASTWalker("testIntField = 0+1");
		Assert.assertEquals(evaluator.evaluate(concept), true);
		evaluator = new ASTWalker("testIntField = -1+2");
		Assert.assertEquals(evaluator.evaluate(concept), true);
		evaluator = new ASTWalker("@id = -1+1");
		Assert.assertEquals(evaluator.evaluate(concept), true);
		evaluator = new ASTWalker("@extId = \"\"+-1+2");
