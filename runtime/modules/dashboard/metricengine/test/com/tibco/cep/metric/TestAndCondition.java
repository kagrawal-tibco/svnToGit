...
public class TestAndCondition {
	...
	public void test() throws Exception {
		...
		evaluator = new ASTWalker("testIntField >= 1 and testStringField = \"tester1\"");
		Assert.assertEquals(evaluator.evaluate(concept), true);
		evaluator = new ASTWalker("testIntField >= 1 and (testStringField = \"tester1\" and testIntField >= 1)");
		Assert.assertEquals(evaluator.evaluate(concept), true);
		evaluator = new ASTWalker("@id >= 0 and (testStringField = \"tester1\" and @id >= 0)");
		Assert.assertEquals(evaluator.evaluate(concept), true);
		evaluator = new ASTWalker("@id >= 0 and (@extId != \"tester1\" and @id >= 0)");
		Assert.assertEquals(evaluator.evaluate(concept), true);
	}
