...
public class TestDivideCondition {
	...
	public void test() throws Exception {
		...
		evaluator = new ASTWalker("testIntField = 2 / 2");
		Assert.assertEquals(evaluator.evaluate(concept), true);
		evaluator = new ASTWalker("testIntField = 10.0 / 10.0");
		Assert.assertEquals(evaluator.evaluate(concept), true);
		evaluator = new ASTWalker("testIntField = 10.0 / 10");
		Assert.assertEquals(evaluator.evaluate(concept), true);
		evaluator = new ASTWalker("testIntField = 4.0 / 4");
		Assert.assertEquals(evaluator.evaluate(concept), true);
		evaluator = new ASTWalker("testIntField = 3 / 2");
		Assert.assertEquals(evaluator.evaluate(concept), true);
		evaluator = new ASTWalker("@id = 0 / 2");
		Assert.assertEquals(evaluator.evaluate(concept), true);
	}
