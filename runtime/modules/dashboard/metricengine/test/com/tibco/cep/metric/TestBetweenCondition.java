...
public class TestBetweenCondition {
	...
	public void test() throws Exception {
		evaluator = new ASTWalker("$testId between 0 and 2");
		Map<String, Object> bindValues = new HashMap<String, Object>();
		bindValues.put("testId", new Integer(1));
		Assert.assertEquals(evaluator.evaluate(concept, bindValues), true);
		evaluator = new ASTWalker("testIntField between 0 and 2");
		Assert.assertEquals(evaluator.evaluate(concept), true);
		evaluator = new ASTWalker("testIntField between -10.0 and 11.0");
		Assert.assertEquals(evaluator.evaluate(concept), true);
		evaluator = new ASTWalker("'2009-12-30' between '2009-12-29T01:01:00.123+0008' and '2009-12-31'");
		Assert.assertEquals(evaluator.evaluate(concept), true);
		evaluator = new ASTWalker("@id between 0 and 2");
		Assert.assertEquals(evaluator.evaluate(concept), true);
	}
