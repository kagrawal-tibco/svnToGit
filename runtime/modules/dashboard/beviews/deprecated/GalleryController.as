package com.tibco.be.views.deprecated{
	...
	import com.tibco.be.views.ui.gallery.view.galleryMetric;
	...
	public class GalleryController implements IEventBusListener{
...
//			PSVRClient.instance.sendRequest(this,galleryModelLoaded,"getcompgallerymodel",null,galleryModelLoadFailed,null);
			...
			EventBus.instance.dispatchEvent(
				new ConfigRequestEvent(ConfigRequestEvent.GET_COMPONENT_GALLERY, this)
			);
		}
		...
		private function xmlLoaded(loadedXml:XMLList):void {
			...
			compGallery.folderItems.dataProvider = currentDirectory.category;
		...
		private function galleryClosed(event:CloseEvent):void {
			EventBus.instance.removeEventListener(ConfigRequestEvent.GET_COMPONENT_GALLERY, handleResponse);
		...
		private function folderClicked(event:MouseEvent):void {
			...
			var metricID:String = event.currentTarget.metricID;
			var prntFolder:String = event.currentTarget.parentID;
			
			trace ("Current Category: " + metricID + " Parent Folder: " + prntFolder);
			if(prntFolder == "ROOT"){
				currentDirectory = new XMLList(fullGalleryXml.category);
			}
			else{
				currentDirectory = new XMLList(currentDirectory.category.(@parentid == metricID));
			}
			...
			addFolderToMenu(folderName, metricID);
		...
		private function upArrowClicked(event:MouseEvent):void {
			...
			if(parentFolder == "ROOT" || parentFolder == null || parentFolder == ""){
				currentDirectory = new XMLList(fullGalleryXml);
				compGallery.folderItems.dataProvider = currentDirectory.category;
				//compGallery.metricItems.dataProvider = currentDirectory.compgallery.category.comp;
			}
			else{
				var parent:XML = currentDirectory.parent() as XML;
				currentDirectory = new XMLList(fullGalleryXml.category.(@parentid == parentFolder));
				...
				parentFolder = fullGalleryXml.category.(@id == parentFolder).@parentid;
			}
		...
		public function handleResponse(response:ConfigResponseEvent):void{
			try{
				...
				if(response.failMessage != ""){
					EventBus.instance.dispatchEvent( new DefaultLogEvent("warning", response.failMessage) );
					return;
				}
				if(response.command == "getcompgallerymodel"){
					galleryModelLoaded(true, new XMLList(response.data));
				}
			}
			catch(error:Error){
				EventBus.instance.dispatchEvent(
					new DefaultLogEvent(
						"critical",
						"GalleryController.hanldeResponse - " + error.message)
				);
			}
		}
