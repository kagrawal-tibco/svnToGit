...
public class XMLUtil {
	...
	/**
	 * Parse an inputstream to a XML Document object
	 * 
	 * @param stream
	 * @return
	 * @throws Exception
	...
	public static Node parse(InputStream stream) throws Exception {
		Document doc = parseToDocument(stream);
		...
		// Now do the includes. Right now, we don't support nested  includes.
	...
	/**
	 * Parse an xml doc of type docType (the starting node type) and return a
	 * node to the root.
	 */
	...
	public static Node parse(String s) throws Exception {
		try {
			ByteArrayInputStream byteStream = new ByteArrayInputStream(s.getBytes(Charset.forName("UTF-8")));
	 ...
	 * 
	 * @param path
	 *            points to an element (in which case the 'content' of that
	 *            element is returned), or an attribute (if prefixed by @, in
	 *            which case the value of the attribute is returned).
	 */
	...
	/**
	 * Return an iteration of specified nodes reachable from node n, by path
	 * path.
	 */
	...
	/**
	 * Return a list of specified nodes reachable from node n, by the path,
	 * filtered by the attribute name and value.
	 */
	...
	/**
	 * Return the node specified by path, filtered by the attribute name and
	 * value.
	 */
	...
	/**
	 * Return the node specified by path, filtered by multiple attribute name
	 * and value pairs.
	 */
	public static Node getNodeByAttribute(Node node, String path, String[] attrName, String[] attrValue) {
	...
	/**
	 * Find all nodes containing the attribute name, or having the attribute
	 * with the value.
	 */
	...
	/**
	 * Set the attribute of a node, identify by path. E.g.
	 * path1/path2/@attribute.
	 * 
	 * @param node
	...
	/**
	 * Create the node tree based on the path if the nodes don't exist, or if
	 * forced.
	 * 
	 * @param node
	 *            the base node
	...
	/**
	 * Return a simple, single line string representation of the node (and its
	 * children). Warning: Note that this method does not escape node contents.
	 */
	...
	private static void doToSimpleString(Node node, StringBuilder sb, String indent, String nl) {
		sb.append(indent + "<" + node.getNodeName());
		NamedNodeMap attrs = node.getAttributes();
		for (int i = 0; i < attrs.getLength(); ++i) {
			String name = attrs.item(i).getNodeName();
	...
	public static Node createElementWithXmlString(Document targetdoc, String xmlString) throws Exception {
		...
		return targetdoc.importNode(doc.getDocumentElement(), true);
	}
