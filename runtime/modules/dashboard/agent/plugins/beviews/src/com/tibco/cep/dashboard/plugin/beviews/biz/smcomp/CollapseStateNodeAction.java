...
public class CollapseStateNodeAction extends BaseAuthenticatedAction {
	...
	protected BizResponse doAuthenticatedExecute(SecurityToken token, BizRequest request) {
		...
		try{
			...
			if (component == null) {
				...
				if (component == null) {
					return handleError(getMessage("collapsestatenode.nonexistent.componentid", getMessageGeneratorArgs(token, componentid)));
				}
			}
			...
			return handleError(getMessage("collapsestatenode.config.generation.failure", getMessageGeneratorArgs(token, e, componentid)), e);
