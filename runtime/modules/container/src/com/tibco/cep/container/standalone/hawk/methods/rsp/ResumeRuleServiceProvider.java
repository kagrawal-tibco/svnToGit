 ...
 */
public class ResumeRuleServiceProvider extends AmiMethod {


    protected HawkRuleAdministrator m_hma;


    public ResumeRuleServiceProvider(HawkRuleAdministrator hma) {

            super("resumeRuleServiceProvider", "Resumes the RuleServiceProvider", AmiConstants.METHOD_TYPE_ACTION);
        this.m_hma = hma;
    }//constr
    ...
    public AmiParameterList onInvoke(AmiParameterList inParams) throws AmiException {
        try {
            ...
            if(rsp instanceof RuleServiceProviderImpl) {
              ((RuleServiceProviderImpl)rsp).resumeRSP();  
            }
            this.m_hma.getLogger().log(Level.INFO, "RuleServiceProvider %s has been resumed", rsp.getName());
        } catch (Exception e) {
