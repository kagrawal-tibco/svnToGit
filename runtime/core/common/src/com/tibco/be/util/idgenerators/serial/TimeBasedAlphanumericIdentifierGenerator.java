...
public class TimeBasedAlphanumericIdentifierGenerator extends AbstractStringIdentifierGenerator
    ...
    public String nextStringIdentifier() {
        long now;
        synchronized (this) {
            now = Calendar.getInstance(com.tibco.be.util.idgenerators.serial.TimeBasedAlphanumericIdentifierGenerator.UTC).getTime().getTime(); // JDK 1.3 compatibility
            final long diff = now - com.tibco.be.util.idgenerators.serial.TimeBasedAlphanumericIdentifierGenerator.last;
            // external time correction of more than a second or overflow
            if (diff > 0 || diff < -1000) {
                com.tibco.be.util.idgenerators.serial.TimeBasedAlphanumericIdentifierGenerator.last = now;
                com.tibco.be.util.idgenerators.serial.TimeBasedAlphanumericIdentifierGenerator.counter = 0;
            } else {
                if (diff != 0) {
                    now = com.tibco.be.util.idgenerators.serial.TimeBasedAlphanumericIdentifierGenerator.last; // ignore time shift
                }
                ++com.tibco.be.util.idgenerators.serial.TimeBasedAlphanumericIdentifierGenerator.counter;
            }
        }
        final String postfix = com.tibco.be.util.idgenerators.serial.TimeBasedAlphanumericIdentifierGenerator.counter > 0
                                          ? Long.toString(com.tibco.be.util.idgenerators.serial.TimeBasedAlphanumericIdentifierGenerator.counter, ALPHA_NUMERIC_CHARSET_SIZE) : "";
        ...
        if (maxPad > 0) {
            System.arraycopy(com.tibco.be.util.idgenerators.serial.TimeBasedAlphanumericIdentifierGenerator.padding, 0, buffer, 0, maxPad);
        }
        ...
        if (postfixSize > 0) {
            ...
            if (maxPad > 0) {
                System.arraycopy(com.tibco.be.util.idgenerators.serial.TimeBasedAlphanumericIdentifierGenerator.padding, 0, buffer, i, maxPad);
                i += maxPad;
            }
