...
/**
 * A modified version of Properties that meet t-tests specfic needs.<br>
 * Special behavior of this class compare to regular Properties are: <br>
 * 1. It is sorted.<br>
 * 2. If value is a file path, then path separator are converted to forward slash("/"). <br>
 * 3. Convert to String[]. <br>
 * 4. Use cache when convert to String[] for performance. <br>
 * 5. Convert to and from String. <br>
 * 6. does not treat ":" as key value separator. <br>
 * //  private static final String specialSaveChars = "=: \t\r\n\f#!"; <br>
       private static final String specialSaveChars = "= \t\r\n\f#!";
 */
public class TProperties extends TreeMap{

    /**
     * Whether or not need to convert to String[] from scratch or merge the newly modified properties.
     */
    private boolean needToRegenerateEnv = true;

    /**
     * Cache for converted String[].
     */
    private String[] currentEnvStrArray = null;

    /**
     * Modifed properties since last convert.
     */
    private TreeMap diffMap = null;

    /**
     * Global variable prefix.
     */
    public static final String TIB_ENV_PREFIX = "tibco.env.";

    public TProperties(String propsString){
        super();
        try {
            load(new ByteArrayInputStream(propsString.getBytes()));
        } catch (IOException e) {
...
//            logger.debug("match \\ " + value);
    ...
    public static String[] getStringArray(TreeMap map) {
        if (map == null || map.size() == 0 ) {
            return null;
        }

        /*
        // no easy conversion , has to iterate and manually convert them
        Vector b = new Vector(map.entrySet());
        b.copyInto(currentEnvStrArray);
        */

        String[] envStrArray = new String[map.size()];
        int index = 0;
        for (Iterator iterator = map.entrySet().iterator(); iterator.hasNext();) {
            Map.Entry pair = (Map.Entry) iterator.next();
            envStrArray[index++] = pair.getKey() + "=" + pair.getValue();
        }
        return envStrArray;
    }
    ...
    public String[] toStringArray() {
        ...
        //disable mergeEnv optimization
        currentEnvStrArray = TProperties.getStringArray(this);
        return currentEnvStrArray;
        /*

        if (currentEnvStrArray == null || needToRegenerateEnv == true) {
//            logger.debug("does a deep conversion to string array");
            currentEnvStrArray = TProperties.getStringArray(this);

        } else {
            if (diffMap != null && diffMap.size() > 0) {
//                logger.debug("does a appending to string array");
                mergeEnv();
                diffMap.clear();
            }
        }
        ...
        if (diffMap != null && diffMap.size() > 0) {
            //this can be true, for example, in Preferences config apply(), setProperty() casue diffMap to have one entry, then
            //setArrayProperty() cause needToRegenerateEnv = true. Then you have both needToRegenerateEnv = true and
            //diffMap.size() > 0. As long as in a round of properties change, put() first and remove() later, you will have
            //this condition

//            logger.error("diffMap has diff properties that have not been clear");
            diffMap.clear();
        }

        needToRegenerateEnv = false;
        return currentEnvStrArray;
        */
    }
    ...
    public synchronized Object put(Object keyObj, Object valueObj) {
        ...
        String value = normalize(valueObj.toString());
        return super.put(key, value);
    }

                  /*
    private void mergeEnv() {
        String[] oldEnv = currentEnvStrArray;
        String[] diffEnv = TProperties.getStringArray(diffMap);

        //TODO be careful when the same env defined multiple times, shell take the last one
        //therefore need to put diff last as it represents the latest change

        currentEnvStrArray = new String[oldEnv.length + diffEnv.length];

        System.arraycopy(oldEnv ,0,currentEnvStrArray, 0, oldEnv.length);
        System.arraycopy(diffEnv,0,currentEnvStrArray, oldEnv.length             , diffEnv.length);

//        logger.debug("old Env, first and last one ");
//        logger.debug("[0] :"  + oldEnv[0] );
//        logger.debug("[" + (oldEnv.length - 1) + "] :"  + oldEnv[oldEnv.length - 1] );
//
//        logger.debug("diff Env, first one and last one");
//        logger.debug("[0] :"  + diffEnv[0]);
//        logger.debug("[" + (diffEnv.length - 1) + "] :"  + diffEnv[diffEnv.length - 1]);
//
//
//        logger.debug("current, several ones");
//        logger.debug("[0] :"  + currentEnvStrArray[0]);
//        logger.debug("[" + (oldEnv.length - 1) + "] :"  + currentEnvStrArray[oldEnv.length - 1]);
//        logger.debug("[" + (oldEnv.length) + "] :"  + currentEnvStrArray[oldEnv.length]);
//        logger.debug("[" + (currentEnvStrArray.length -1 ) + "] :"  + currentEnvStrArray[currentEnvStrArray.length -1 ]);
    ...
    */
    static Logger logger = Logger.getLogger(TProperties.class);
...
//  private static final String specialSaveChars = "=: \t\r\n\f#!";
     ...
     *
href="http://java.sun.com/docs/books/jls/second_edition/html/lexical.doc.htm
l#100850">&sect;3.3</a>
     * and <a
     *
href="http://java.sun.com/docs/books/jls/second_edition/html/lexical.doc.htm
l#101089">&sect;3.10.6</a>
     ...
     * @exception  java.io.IOException  if an error occurred when reading
from the
     *               input stream.
...
startIndex++)
                            if
(whiteSpaceChars.indexOf(nextLine.charAt(startIndex)) == -1)
                                break;
                        nextLine =
...
line.substring(valueIndex, len) : "";
     ...
     * occurs while saving the property list.  As of the Java 2 platform
v1.2, the preferred
     * way to save a properties list is via the <code>store(OutputStream
out,
...
<code>Strings</code>.
     ...
     * <code>Properties</code> table to the output stream in a format
suitable
     * for loading into a <code>Properties</code> table using the
     * {@link #load(java.io.InputStream) load} method.
...
The
     ...
     * @param   out      an output stream.
     * @param   header   a description of the property list.
     * @exception  java.io.IOException if writing this property list to the
specified
...
<code>Strings</code>.
    ...
    throws IOException
    {
        ...
        String[] envArray = toStringArray();
        if (envArray != null){
            for (int i = 0; i < envArray.length; i++) {
                writeln(awriter, envArray[i]);
            }
        }
...
//        }
    ...
    private static void writeln(BufferedWriter bw, String s) throws
IOException {
        bw.write(s);
        bw.newLine();
    }
...
//	    String key = (String)e.nextElement();
    ...
    public synchronized void toEnvScript(OutputStream out, String header)
    throws IOException
    {
...
//            String val = (String)get(key);
    ...
    public String toString(){
        ...
        try {
            store(bos, "System TProperties pass to EngineWrapper");
            result = bos.toString();
        } catch (IOException e) {
            logger.error(e.getMessage(), e);
        }
