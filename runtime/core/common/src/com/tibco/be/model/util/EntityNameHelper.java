...
public class EntityNameHelper {
    ...
    protected static final HashSet KEYWORDS = new HashSet(Arrays.asList(new String[] {
...
//    }
    ...
    public static String isValidEntityPath(String string, boolean allowKeywords, boolean isLinkedResource) {
        if(!string.startsWith(("" + Folder.FOLDER_SEPARATOR_CHAR))) return null;

        String[] entities = string.split("" + Folder.FOLDER_SEPARATOR_CHAR);
        for(int i = 0; i < entities.length; i++) {
            if (entities[i].length() == 0) continue;
            if (!isValidEntityIdentifier(entities[i]) && !isLinkedResource) return entities[i];
            if (!allowKeywords && isKeyword(entities[i])) return entities[i];
        }
    ...
    public static boolean containsInvalidNameChars(String name, boolean allowPathSeparator) {
        return containsInvalidNameChars(name, allowPathSeparator, false, false);
    }
     ...
     */
    public static boolean containsInvalidNameChars(String name, boolean allowPathSeparator, boolean allowSpaces, boolean isLinkedResource) {
        ...
        for(int i = 0; i < name.length(); i++) {
            ...
            boolean invalidPart = (ch != Folder.FOLDER_SEPARATOR_CHAR) && (!ModelNameUtil.isIdentifierPart(ch)) && !isLinkedResource;
