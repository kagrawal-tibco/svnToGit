...
public class MetadataCache {
        ...
        for ( ;recoverState != MetadataRecoverState.recovered; ) {
            ...
            switch (recoverState) {
                ...
                case recovering: {
                    ...
                    boolean lockAcquired = recoveryCoordinatorDao.lock(sharedStateKey, 10000);
                    if (lockAcquired) {
                        recoveryCoordinatorDao.unlock(sharedStateKey);
                    }
                    Thread.sleep(5000);
                    continue;
                }
                ...
                default: {
                    ...
                    try {
                        ...
                        if (recoverState == MetadataRecoverState.recovered) {
                            ...
                            recoveryCoordinatorDao.put(sharedStateKey, MetadataRecoverState.recovering);
                            boolean success = recover();
                            if (success) {
                            	recoverState = MetadataRecoverState.recovered;
                            	recoveryCoordinatorDao.put(sharedStateKey, recoverState);
                            } else {
                            	recoveryCoordinatorDao.remove(sharedStateKey);
                            }
                        }
                    } finally {
                        recoveryCoordinatorDao.unlock(sharedStateKey);
    ...
    protected boolean recover() {
    	try {
	        ...
	        LinkedHashMap<String, Integer> typeRegistry = null;
	        if (store != null && store instanceof BackingStore) {
	            typeRegistry = ((BackingStore) store).getClassRegistry();
	        }
    ...
    protected void loadTypes() throws Exception {
        ...
        if (recoverSpaces == false) {
            cluster.getDaoProvider().recoverCluster(cluster);
        }
     ...
     * @return
     */
    public EntityDao createEntityDaoAndRegister(Class entityClass, String uri, int typeId) {
        EntityDao entityDao;
        ...
        for (Map.Entry entry : entries) {
            ...
            if (clz != null) {
                if ((type.getType() != TypeManager.TYPE_RULE) && (type.getType() != TypeManager.TYPE_RULEFUNCTION)) {
                    ...
                    if (objTypeId == null) {
                        typeId = ++lastTypeId;
                        if (backingStoreTypeMap != null) backingStoreTypeMap.put(clzName, typeId);
                        typeIdDao.put(URI, typeId);
                    } else {
                        typeId = objTypeId;
     ...
     * @throws Exception
     */
    public com.tibco.cep.kernel.model.entity.Entity createEntity(int typeId, long id, String extId) throws Exception {
    ...
    protected static class MetadataDescriptor {
        ...
        @Optional
        protected Constructor entityConstructorTwoArgs = null;
