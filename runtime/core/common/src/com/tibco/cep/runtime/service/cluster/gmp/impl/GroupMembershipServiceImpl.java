...
public class GroupMembershipServiceImpl implements GroupMembershipService {
	...
	protected ControlDao masterDao;
	...
	public void init(Cluster cluster) {
		...
		masterDao = cluster.getDaoProvider().createControlDao(Integer.class, byte[].class, ControlDaoType.Master);
	}
	...
	public void lock() {
		...
		try {
			for (;;) {
				acquireLock = masterDao.lock(key, lockWaitTime);
				if (acquireLock) {
					if (logger.isEnabledFor(lockLogLevel)) {
