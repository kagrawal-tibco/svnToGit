...
import com.tibco.cep.runtime.service.cluster.scheduler.impl.DefaultTaskScheduler;
...
public class DefaultSchedulerCache implements MapChangeListener<String, WorkManagerEntry>, SchedulerCache, GroupMemberServiceListener {
    ...
    private boolean bestoreManagedWorklist = Boolean.parseBoolean(System.getProperty(SystemProperty.STORE_MANAGED_WORKLIST.getPropertyName(), "true"));
...
//    ASDaoProvider daoProvider;
    ...
    public void init(Cluster cluster) throws Exception {
        ...
        this.beStore = cluster.getBackingStore();
//        ASClusterProvider clusterProvider = (ASClusterProvider) cluster.getClusterProvider();
//        this.daoProvider = clusterProvider.getDaoProvider();
        ...
        workManagerDao = cluster.getClusterProvider().createControlDao(String.class, WorkManagerEntry.class, ControlDaoType.WorkManager, cluster);
...
//    }
            ...
            if (entry != null) {
                if (entry.getNodeId() == null) {
                    ...
                    if (schedulerAgent != null) {
                        ...
                        DefaultTaskScheduler scheduler = new DefaultTaskScheduler(this, entry, schedulerAgent, true);
    ...
    public void schedule(String schedulerId, String workKey, WorkEntry entry) throws Exception {
        ...
        ControlDao<String, WorkTuple> workList = bestoreManagedWorklist ? null : getSchedulerWorkList(schedulerId);
        if (bestoreManagedWorklist || workList != null) {
            ...
            work.setWork(entry);
            if (bestoreManagedWorklist) {
            	beStore.saveWorkItem(work);
            } else if (cluster.getClusterConfig().isHasBackingStore() && cluster.getClusterConfig().isCacheAside()) {
            	CacheAsideBackingStore backingStore = cluster.getBackingStore();
    ...
    @Override
    public ControlDao<String, WorkTuple> getSchedulerWorkList(String schedulerId) throws Exception {
        if (bestoreManagedWorklist)
        	return null;
        
        ControlDao<String, WorkTuple> dao = schedulerDaos.get(schedulerId);
        if (dao == null) {
            dao = cluster.getClusterProvider().createControlDao(String.class, WorkTuple.class, ControlDaoType.WorkList$SchedulerId, cluster, schedulerId);
    ...
    */
    ...
    public WorkEntry getEntry(String schedulerId, String workKey) throws Exception {
        ...
        if (entry != null) {
            if (cluster.getClusterConfig().isHasBackingStore()) {
                ...
                if (beStore != null) {
                    WorkTuple tuple = beStore.getWorkItem(schedulerId, workKey);
                    if (tuple != null) {
    ...
    public WorkEntry remove(String schedulerId, String workKey) throws Exception {
        ...
        if (entry != null) {
            if (cluster.getClusterConfig().isHasBackingStore()) {
                ...
                GenericBackingStore backingStore = cluster.getBackingStore();
                if (backingStore != null && (backingStore instanceof CacheAsideBackingStore)) {
                    ((CacheAsideBackingStore)backingStore).removeWorkItem(key);
                }
            }
            ControlDao<String, WorkTuple> worklist = bestoreManagedWorklist ? null : getSchedulerWorkList(schedulerId);
            if (bestoreManagedWorklist) {
            	beStore.removeWorkItem(schedulerId + "." + workKey);
    ...
    public void remove(String schedulerId, Set<String> workKeys) throws Exception {
    	if(bestoreManagedWorklist) {
    		for (String s : workKeys) {
    			beStore.removeWorkItem(schedulerId + "." + s);//TODO: BEStore APIs should accept schedulerId as separate param.
    		}
    		return;
    	}
        ControlDao<String, WorkTuple> worklist = getSchedulerWorkList(schedulerId);
    ...
    public void onPut(String schedulerKey, WorkManagerEntry entry, boolean isLocal) {
    ...
    public void onRemove(String key, WorkManagerEntry value, boolean isLocal) {
    ...
    @Override
	public Map<String, WorkManagerEntry> getAllSchedulers() {
		Map<String, WorkManagerEntry> wmMap = new LinkedHashMap<String, WorkManagerEntry>();
		for (Map.Entry<String, WorkManagerEntry> e : workManagerDao.entrySet()) {
			wmMap.put(e.getKey(), e.getValue());
		}
		return wmMap;
	}

	@Override
	public WorkManagerEntry get(String schedulerId) {
		return workManagerDao.get(schedulerId);
	}
