...
public final class InvokeJavaCustomMethodXFunction extends InvokeJavaXFunction
{
    ...
    {
    	super(null, ExpandedName.makeName(namespace, method.getFnName()));
...
//    	super(method.getFnName(), getReturnType(method), getArgTypes(method));
    ...
    public SmSequenceType getReturnType() {
    	return getReturnType(mInfo);
    }
    
    public static SmSequenceType getReturnType(FunctionInfo method) {
	...
	private static SmSequenceType[] getArgTypes(FunctionInfo method) {
		List<ParamTypeInfo> paramTypes = method.getArgs();
		if (paramTypes == null) {
			return new SmSequenceType[0];
		}
		SmSequenceType[] types = new SmSequenceType[paramTypes.size()];
