 ...
 */
public class OracleConceptSerializerImpl implements ConceptSerializer, OracleSerializer {
     ...
     *
     */
    public OracleConceptSerializerImpl(OracleConceptMap entityDescription, OracleConnection oracle, String qual) {
    ...
    public void startConcept(Class clz, long key, String extKey, int state,int version) {
        try {
            if (!error) {
                ...
                attributes[3]= new oracle.sql.TIMESTAMP(new java.sql.Timestamp(System.currentTimeMillis()));
                attributes[4]= new oracle.sql.TIMESTAMP(new java.sql.Timestamp(System.currentTimeMillis()));
            }
        } catch (Exception ex) {
            error=true;
            msg= ex.getMessage();
    ...
    void setCurrentDatum(Datum[] attrs) throws SQLException {
        if (!inArray) {
            ...
            if (ad.getBaseType() == OracleTypes.STRUCT) {
                arrayAttributes[arrayCursor]= new oracle.sql.STRUCT((StructDescriptor) StructDescriptor.getTypeDescriptor(ad.getBaseName(), oracle), oracle, attrs);
            }
        }
    }
    /**
    ...
    STRUCT historyTable(Datum[] attributes) throws Exception{
        ...
        if (inArray) {
            ...
            StructDescriptor sd= (StructDescriptor) getStructDescriptor(ad.getBaseName(), oracle);
