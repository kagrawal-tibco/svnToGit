...
if %AS_VERSION% EQU na (
	powershell -Command "(Get-Content 'TIBCOUniversalInstaller_businessevents-enterprise_%BE_PRODUCT_VERSION%.silent') -replace '<entry key=\"feature_TIBCO BusinessEvents DataGrid SP 1_businessevents-enterprise\">true</entry>', '<entry key=\"feature_TIBCO BusinessEvents DataGrid SP 1_businessevents-enterprise\">false</entry>' | Set-Content 'TIBCOUniversalInstaller_businessevents-enterprise_%BE_PRODUCT_VERSION%.silent'"
)
